...
public class XSSFTextParagraph implements Iterable<XSSFTextRun> {
    ...
    public Color getBulletFontColor() {
        ...
        if (bytes.length == 3) {
            return new Color(bytes[0] & UByte.MAX_VALUE, bytes[1] & UByte.MAX_VALUE, bytes[2] & UByte.MAX_VALUE);
        }
        return new Color(bytes[1] & UByte.MAX_VALUE, bytes[2] & UByte.MAX_VALUE, bytes[3] & UByte.MAX_VALUE, bytes[0] & UByte.MAX_VALUE);
    }

    public byte[] getBulletFontColorAsBytes() {
        ParagraphPropertyFetcher<byte[]> fetcher = new ParagraphPropertyFetcher<byte[]>(getLevel()) { // from class: org.apache.poi.xssf.usermodel.XSSFTextParagraph.5
            @Override // org.apache.poi.xssf.model.ParagraphPropertyFetcher
            public boolean fetch(CTTextParagraphProperties props) {
        ...
        ParagraphPropertyFetcher<Double> fetcher = new ParagraphPropertyFetcher<Double>(getLevel()) { // from class: org.apache.poi.xssf.usermodel.XSSFTextParagraph.6
            ...
            public boolean fetch(CTTextParagraphProperties props) {
                if (props.isSetBuSzPct()) {
                    setValue(Double.valueOf(POIXMLUnits.parsePercent(props.getBuSzPct().xgetVal()) * 0.001d));
                    return true;
                }
    ...
    public void setLineSpacing(double linespacing) {
        ...
        if (linespacing >= 0.0d) {
            spc.addNewSpcPct().setVal(Integer.valueOf((int) (1000.0d * linespacing)));
        } else {
    ...
    public double getLineSpacing() {
        ...
        ParagraphPropertyFetcher<Double> fetcher = new ParagraphPropertyFetcher<Double>(getLevel()) { // from class: org.apache.poi.xssf.usermodel.XSSFTextParagraph.12
            ...
            public boolean fetch(CTTextParagraphProperties props) {
                if (props.isSetLnSpc()) {
                    ...
                    setValue(Double.valueOf(POIXMLUnits.parsePercent(spc.getSpcPct().xgetVal()) * 0.001d));
                    return true;
                }
                return false;
            }
        ...
        if (lnSpc > 0.0d && (normAutofit = this._shape.getTxBody().getBodyPr().getNormAutofit()) != null && normAutofit.isSetLnSpcReduction()) {
            double scale = 1.0d - (NumberHelper.toDouble(normAutofit.getLnSpcReduction()) / 100000.0d);
            return lnSpc * scale;
        }
        return lnSpc;
    }
    ...
    public void setSpaceBefore(double spaceBefore) {
        ...
        CTTextSpacing spc = CTTextSpacing.Factory.newInstance();
        if (spaceBefore >= 0.0d) {
            spc.addNewSpcPct().setVal(Integer.valueOf((int) (1000.0d * spaceBefore)));
        } else {
    ...
    public void setSpaceAfter(double spaceAfter) {
        ...
        CTTextSpacing spc = CTTextSpacing.Factory.newInstance();
        if (spaceAfter >= 0.0d) {
            spc.addNewSpcPct().setVal(Integer.valueOf((int) (1000.0d * spaceAfter)));
        } else {
