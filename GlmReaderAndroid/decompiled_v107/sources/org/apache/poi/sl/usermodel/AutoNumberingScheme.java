...
public enum AutoNumberingScheme {
    ...
    private String formatSeparator(String cased) {
        ...
        return name.contains("plain") ? cased : name.contains("parenright") ? cased + ")" : name.contains("parenboth") ? "(" + cased + ")" : name.contains(TypedValues.CycleType.S_WAVE_PERIOD) ? cased + "." : name.contains("minus") ? cased + ProcessIdUtil.DEFAULT_PROCESSID : cased;
    }
    ...
    private String formatRomanIndex(int value) {
        int[] VALUES = {1000, TypedValues.Custom.TYPE_INT, 500, FontHeader.REGULAR_WEIGHT, 100, 90, 50, 40, 10, 9, 5, 4, 1};
