...
public class POIFSMiniStore extends BlockStore {
    ...
    public POIFSMiniStore(POIFSFileSystem filesystem, RootProperty root, List<BATBlock> sbats, HeaderBlock header) {
        if (root == null) {
    ...
    public ByteBuffer getBlockAt(int offset) throws IOException {
        ...
        ByteBuffer dataBlock = this._filesystem.getBlockAt(it.next().intValue());
        if (dataBlock == null) {
    ...
    public void syncWithDataSource() throws IOException {
        ...
        this._filesystem._get_property_table().getRoot().setSize(computeSize());
    }

    private int computeSize() {
        int entriesPerBlock = this._filesystem.getBigBlockSizeDetails().getBATEntriesPerBlock();
        for (int sbatIndex = this._sbat_blocks.size() - 1; sbatIndex >= 0; sbatIndex--) {
            BATBlock sbat = this._sbat_blocks.get(sbatIndex);
