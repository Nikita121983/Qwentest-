...
public class DataSpaceMapUtils {
    ...
    public static DocumentEntry createEncryptionEntry(DirectoryEntry dir, String path, EncryptionRecord out) throws IOException {
        ...
        String[] parts = path.split(PackagingURIHelper.FORWARD_SLASH_STRING);
        for (int i = 0; i < parts.length - 1; i++) {
            if (dir.hasEntryCaseInsensitive(parts[i])) {
                createDirectory = (DirectoryEntry) dir.getEntryCaseInsensitive(parts[i]);
            } else {
                createDirectory = dir.createDirectory(parts[i]);
            }
            dir = createDirectory;
        }
        ...
        return dir.createDocument(fileName, bos.getWriteIndex(), new POIFSWriterListener() { // from class: org.apache.poi.poifs.crypt.DataSpaceMapUtils$$ExternalSyntheticLambda0
            ...
            public final void processPOIFSWriterEvent(POIFSWriterEvent pOIFSWriterEvent) {
                DataSpaceMapUtils.lambda$createEncryptionEntry$0(buf, pOIFSWriterEvent);
            }
        });
    }

    /* JADX INFO: Access modifiers changed from: package-private */
    public static /* synthetic */ void lambda$createEncryptionEntry$0(byte[] buf, POIFSWriterEvent event) {
        try {
