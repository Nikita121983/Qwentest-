...
public abstract class RowShifter extends BaseRowColShifter {
    ...
    private boolean removalNeeded(CellRangeAddress merged, int startRow, int endRow, int n) {
        ...
        if (n > 0) {
            ...
            int firstRow2 = startRow + n;
            int lastRow2 = Math.min(startRow - 1, startRow + n + movedRows);
            overwrite = new CellRangeAddress(firstRow2, lastRow2, merged.getFirstColumn(), merged.getLastColumn());
        }
    ...
    public static void validateShiftParameters(int firstShiftColumnIndex, int lastShiftColumnIndex, int step) {
        ...
        if (firstShiftColumnIndex > lastShiftColumnIndex) {
            throw new IllegalArgumentException(String.format(LocaleUtil.getUserLocale(), "Incorrect shifting range : %d-%d", Integer.valueOf(firstShiftColumnIndex), Integer.valueOf(lastShiftColumnIndex)));
        }
    }
