...
public final class PageSettingsBlock extends RecordAggregate {
    ...
    private static void shiftBreaks(PageBreakRecord breaks, int start, int stop, int count) {
        ...
        for (PageBreakRecord.Break breakItem2 : shiftedBreak) {
            breaks.removeBreak(breakItem2.getMain());
            breaks.addBreak((short) (breakItem2.getMain() + count), breakItem2.getSubFrom(), breakItem2.getSubTo());
        }
    }
    ...
    public void positionRecords(List<RecordBase> sheetRecords) {
        ...
        for (RecordBase rb : sheetRecords) {
            if (rb instanceof CustomViewSettingsRecordAggregate) {
                ...
                cv.visitContainedRecords(new RecordAggregate.RecordVisitor() { // from class: org.apache.poi.hssf.record.aggregates.PageSettingsBlock$$ExternalSyntheticLambda0
                    @Override // org.apache.poi.hssf.record.aggregates.RecordAggregate.RecordVisitor
                    public final void visitRecord(Record record) {
                        PageSettingsBlock.this.m2401xb3fdafcc(hfGuidMap, cv, record);
                    }
                });
            }
        }
    ...
    public /* synthetic */ void m2401xb3fdafcc(Map hfGuidMap, CustomViewSettingsRecordAggregate cv, Record r) {
        if (r.getSid() == 426) {
