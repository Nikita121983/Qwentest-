...
public final class LinkTable {
        ...
        public CRNBlock(RecordStream rs) {
            ...
            for (int i = 0; i < crns.length; i++) {
                ...
                if (!(record instanceof CRNRecord)) {
                    throw new IllegalStateException("Record is not a CRNRecord: " + (record == null ? "<null>" : record.getClass() + ": " + record));
                }
    ...
    private static ExternSheetRecord readExtSheetRecord(RecordStream rs) {
        ...
        if (nItems < 1) {
            throw new IllegalStateException("Expected an EXTERNSHEET record but got (" + rs.peekNextClass().getName() + ")");
        }
        if (nItems == 1) {
            return temp.get(0);
        }
    ...
    public LinkTable(int numberOfSheets, WorkbookRecordList workbookRecordList) {
        ...
        if (idx < 0 && findFirstRecordLocBySid(SSTRecord.sid) - 1 < 0) {
    ...
    public int checkExternSheet(int firstSheetIndex, int lastSheetIndex) {
        ...
        if (this._externSheetRecord == null) {
            throw new IllegalStateException("Did not have an external sheet record, having blocks: " + this._externalBookBlocks.length);
        }
