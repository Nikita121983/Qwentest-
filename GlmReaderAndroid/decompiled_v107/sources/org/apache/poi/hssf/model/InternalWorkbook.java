...
public final class InternalWorkbook {
    ...
    public static final List<String> WORKBOOK_DIR_ENTRY_NAMES_CASE_INSENSITIVE = Collections.unmodifiableList(Arrays.asList(WORKBOOK, BOOK));
    private static final Logger LOG = PoiLogManager.getLogger((Class<?>) InternalWorkbook.class);
    ...
    */
    ...
    public FontRecord getFontRecordAt(int idx) {
        ...
        Record record = this.records.get((this.records.getFontpos() - (this.numfonts - 1)) + index);
    ...
    public void setSheetBof(int sheetIndex, int pos) {
        LOG.atDebug().log("setting bof for sheetnum ={} at pos={}", Unbox.box(sheetIndex), Unbox.box(pos));
    ...
    public ExtendedFormatRecord getExFormatAt(int index) {
        ...
        Record record = this.records.get(xfptr + index);
    ...
    public UnicodeString getSSTString(int str) {
        ...
        LOG.atTrace().log("Returning SST for index={} String= {}", Unbox.box(str), retval);
        return retval;
    }
    ...
    public void preSerialize() {
        if (this.records.getTabpos() > 0) {
            ...
            if (rec instanceof TabIdRecord) {
                TabIdRecord tir = (TabIdRecord) rec;
                if (tir.getTabIdSize() < this.boundsheets.size()) {
                    fixTabIdRecord();
                }
            }
        }
    ...
    public NameRecord createBuiltInName(byte builtInName, int sheetNumber) {
        ...
        if (this.linkTable.nameAlreadyExists(name)) {
            throw new IllegalStateException("Builtin (" + ((int) builtInName) + ") already exists for sheet (" + sheetNumber + ")");
        }
        addName(name);
        return name;
    }
    ...
    public DrawingManager2 findDrawingGroup() {
        ...
        this.drawingManager = findDrawingManager((DrawingGroupRecord) findFirstRecordBySid(DrawingGroupRecord.sid), this.escherBSERecords);
        return this.drawingManager;
    }
    ...
    private static DrawingManager2 findDrawingManager(DrawingGroupRecord dg, List<EscherBSERecord> escherBSERecords) {
        ...
        Iterator<EscherRecord> it = cr.iterator();
        while (it.hasNext()) {
        ...
        if (bStore != null) {
            Iterator<EscherRecord> it2 = bStore.iterator();
            while (it2.hasNext()) {
                EscherRecord bs = it2.next();
    ...
    public void createDrawingGroup() {
        if (this.drawingManager == null) {
            ...
            opt.addEscherProperty(new EscherRGBProperty(EscherPropertyTypes.LINESTYLE__COLOR, HSSFShape.LINESTYLE__COLOR_DEFAULT));
    ...
    public void cloneDrawings(InternalSheet sheet) {
        ...
        if (internalWorkbook.drawingManager == null || (aggLoc = sheet.aggregateDrawingRecords(internalWorkbook.drawingManager, false)) == -1) {
            return;
        }
        ...
        while (it.hasNext()) {
            ...
            if (er instanceof EscherDgRecord) {
                ...
                Iterator<EscherRecord> it2 = ((EscherContainerRecord) er).iterator();
                while (it2.hasNext()) {
                    EscherRecord er2 = it2.next();
                    Iterator<EscherRecord> it3 = ((EscherContainerRecord) er2).iterator();
                    while (it3.hasNext()) {
                        EscherRecord shapeChildRecord = it3.next();
                        ...
                        if (recordId == EscherSpRecord.RECORD_ID) {
                            ...
                            if (recordId == EscherOptRecord.RECORD_ID) {
                                ...
                                EscherSimpleProperty prop = (EscherSimpleProperty) opt.lookup(EscherPropertyTypes.BLIP__BLIPTODISPLAY);
                                if (prop != null) {
    ...
    public NameRecord cloneFilter(int filterDbNameIndex, int newSheetIndex) {
        ...
        int i2 = newSheetIndex + 1;
        NameRecord newNameRecord = createBuiltInName((byte) 13, i2);
