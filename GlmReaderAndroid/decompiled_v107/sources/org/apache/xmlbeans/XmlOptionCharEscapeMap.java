...
public class XmlOptionCharEscapeMap {
    ...
    static {
        Map<Character, String> predefEntities = new HashMap<>();
        predefEntities.put(Character.valueOf(Typography.less), "&lt;");
        predefEntities.put(Character.valueOf(Typography.greater), "&gt;");
        predefEntities.put(Character.valueOf(Typography.amp), "&amp;");
        predefEntities.put(Character.valueOf(Chars.QUOTE), "&apos;");
        predefEntities.put('\"', "&quot;");
    ...
    public void addMappings(char ch1, char ch2, int mode) throws XmlException {
        ...
        for (char c = ch1; c <= ch2; c = (char) (c + 1)) {
            addMapping(c, mode);
        }
    }
