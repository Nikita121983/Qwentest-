...
public final class XsTypeConverter {
    ...
    private static final String[] URI_CHARS_TO_BE_REPLACED = {StringUtils.SPACE, VectorFormat.DEFAULT_PREFIX, VectorFormat.DEFAULT_SUFFIX, "|", "\\", "^", CollectionUtils.DEFAULT_TOSTRING_PREFIX, CollectionUtils.DEFAULT_TOSTRING_SUFFIX, "`"};
    ...
    private static int parseIntXsdNumber(CharSequence ch, int min_value, int max_value) {
        ...
        for (int i = 0; i < length - start; i++) {
            int v = Character.digit(ch.charAt(i + start), 10);
            if (v < 0) {
                throw new NumberFormatException("For input string: \"" + ch.toString() + "\"");
            }
