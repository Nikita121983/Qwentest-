...
public class MavenPluginHelp extends AbstractMojo {
    ...
    private Document build() throws MojoExecutionException {
        ...
        try {
            ...
            try {
                Document readDocument = DocumentHelper.readDocument(new XmlOptions(), is);
                if (is != null) {
                    is.close();
                }
                return readDocument;
    ...
    public void execute() throws MojoExecutionException {
        ...
        if (isNotEmpty(name) && !name.contains(id)) {
            append(sb, name + StringUtils.SPACE + version, 0);
        ...
        for (String token : tokens) {
            if (buf.length() > 0) {
                if (buf.length() + token.length() >= lineLength) {
                    ...
                    buf.setLength(0);
                    buf.append(repeat(StringUtils.SPACE, lineIndent * indentSize));
                } else {
            ...
            for (int j = 0; j < token.length(); j++) {
                ...
                if (c == '\t') {
                    buf.append(repeat(StringUtils.SPACE, indentSize - (buf.length() % indentSize)));
                } else if (c == 160) {
                    buf.append(Chars.SPACE);
                } else {
                    buf.append(c);
    ...
    private static int getIndentLevel(String line) {
        ...
        for (int i2 = level + 1; i2 <= level + 4 && i2 < line.length(); i2++) {
            if (line.charAt(i2) == '\t') {
                return level + 1;
            }
        }
    ...
    private String getPropertyFromExpression(String expression) {
        if (expression != null && expression.startsWith("${") && expression.endsWith(VectorFormat.DEFAULT_SUFFIX) && !expression.substring(2).contains("${")) {
