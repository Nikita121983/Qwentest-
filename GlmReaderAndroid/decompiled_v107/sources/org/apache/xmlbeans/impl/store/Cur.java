package org.apache.xmlbeans.impl.store;

import java.io.PrintStream;
import java.util.Map;
import javax.xml.namespace.QName;
import org.apache.commons.collections4.CollectionUtils;
import org.apache.commons.lang3.StringUtils;
import org.apache.xmlbeans.CDataBookmark;
import org.apache.xmlbeans.SchemaType;
import org.apache.xmlbeans.XmlCursor;
import org.apache.xmlbeans.XmlLineNumber;
import org.apache.xmlbeans.XmlObject;
import org.apache.xmlbeans.XmlOptions;
import org.apache.xmlbeans.impl.soap.Detail;
import org.apache.xmlbeans.impl.soap.DetailEntry;
import org.apache.xmlbeans.impl.soap.SOAPBody;
import org.apache.xmlbeans.impl.soap.SOAPBodyElement;
import org.apache.xmlbeans.impl.soap.SOAPElement;
import org.apache.xmlbeans.impl.soap.SOAPEnvelope;
import org.apache.xmlbeans.impl.soap.SOAPFault;
import org.apache.xmlbeans.impl.soap.SOAPFaultElement;
import org.apache.xmlbeans.impl.soap.SOAPHeader;
import org.apache.xmlbeans.impl.soap.SOAPHeaderElement;
import org.apache.xmlbeans.impl.store.DomImpl;
import org.apache.xmlbeans.impl.store.Locale;
import org.apache.xmlbeans.impl.values.TypeStoreUser;
import org.w3c.dom.Document;
import org.w3c.dom.DocumentFragment;

/* loaded from: classes11.dex */
public final class Cur {
    static final /* synthetic */ boolean $assertionsDisabled = false;
    static final int ATTR = 3;
    static final int COMMENT = 4;
    static final int DISPOSED = 3;
    static final int ELEM = 2;
    static final int EMBEDDED = 2;
    static final int END_POS = -1;
    static final int NO_POS = -2;
    static final int POOLED = 0;
    static final int PROCINST = 5;
    static final int REGISTERED = 1;
    static final int ROOT = 1;
    static final int TEXT = 0;
    int _cchSrc;
    String _id;
    Locale _locale;
    Cur _next;
    Cur _nextTemp;
    int _offSrc;
    private int _posTemp;
    Cur _prev;
    Cur _prevTemp;
    Locale.Ref _ref;
    Xobj _xobj;
    int _pos = -2;
    int _tempFrame = -1;
    int _state = 0;
    int _stackTop = -1;
    int _selectionFirst = -1;
    int _selectionN = -1;
    int _selectionLoc = -1;
    int _selectionCount = 0;

    /* JADX INFO: Access modifiers changed from: package-private */
    public Cur(Locale l) {
        this._locale = l;
    }

    public boolean isPositioned() {
        if (isNormal()) {
            return this._xobj != null;
        }
        throw new AssertionError();
    }

    /* JADX INFO: Access modifiers changed from: package-private */
    public static boolean kindIsContainer(int k) {
        return k == 2 || k == 1;
    }

    /* JADX INFO: Access modifiers changed from: package-private */
    public static boolean kindIsFinish(int k) {
        return k == -2 || k == -1;
    }

    public int kind() {
        if (!isPositioned()) {
            throw new AssertionError();
        }
        int kind = this._xobj.kind();
        if (this._pos == 0) {
            return kind;
        }
        if (this._pos == -1) {
            return -kind;
        }
        return 0;
    }

    public boolean isRoot() {
        if (isPositioned()) {
            return this._pos == 0 && this._xobj.kind() == 1;
        }
        throw new AssertionError();
    }

    public boolean isElem() {
        if (isPositioned()) {
            return this._pos == 0 && this._xobj.kind() == 2;
        }
        throw new AssertionError();
    }

    public boolean isAttr() {
        if (isPositioned()) {
            return this._pos == 0 && this._xobj.kind() == 3;
        }
        throw new AssertionError();
    }

    public boolean isComment() {
        if (isPositioned()) {
            return this._pos == 0 && this._xobj.kind() == 4;
        }
        throw new AssertionError();
    }

    public boolean isProcinst() {
        if (isPositioned()) {
            return this._pos == 0 && this._xobj.kind() == 5;
        }
        throw new AssertionError();
    }

    public boolean isText() {
        if (isPositioned()) {
            return this._pos > 0;
        }
        throw new AssertionError();
    }

    public boolean isEnd() {
        if (isPositioned()) {
            return this._pos == -1 && this._xobj.kind() == 2;
        }
        throw new AssertionError();
    }

    public boolean isEndRoot() {
        if (isPositioned()) {
            return this._pos == -1 && this._xobj.kind() == 1;
        }
        throw new AssertionError();
    }

    public boolean isNode() {
        if (isPositioned()) {
            return this._pos == 0;
        }
        throw new AssertionError();
    }

    public boolean isContainer() {
        if (isPositioned()) {
            return this._pos == 0 && kindIsContainer(this._xobj.kind());
        }
        throw new AssertionError();
    }

    public boolean isFinish() {
        if (isPositioned()) {
            return this._pos == -1 && kindIsContainer(this._xobj.kind());
        }
        throw new AssertionError();
    }

    public boolean isUserNode() {
        if (!isPositioned()) {
            throw new AssertionError();
        }
        int k = kind();
        if (k == 2 || k == 1) {
            return true;
        }
        return k == 3 && !isXmlns();
    }

    public boolean isContainerOrFinish() {
        if (!isPositioned()) {
            throw new AssertionError();
        }
        if (this._pos != 0 && this._pos != -1) {
            return false;
        }
        int kind = this._xobj.kind();
        return kind == 2 || kind == -2 || kind == 1 || kind == -1;
    }

    public boolean isNormalAttr() {
        return isNode() && this._xobj.isNormalAttr();
    }

    public boolean isXmlns() {
        return isNode() && this._xobj.isXmlns();
    }

    public boolean isTextCData() {
        return this._xobj.hasBookmark(CDataBookmark.class, this._pos);
    }

    public QName getName() {
        if (isNode() || isEnd()) {
            return this._xobj._name;
        }
        throw new AssertionError();
    }

    public String getLocal() {
        return getName().getLocalPart();
    }

    public String getUri() {
        return getName().getNamespaceURI();
    }

    public String getXmlnsPrefix() {
        if (!isXmlns()) {
            throw new AssertionError();
        }
        return this._xobj.getXmlnsPrefix();
    }

    public String getXmlnsUri() {
        if (!isXmlns()) {
            throw new AssertionError();
        }
        return this._xobj.getXmlnsUri();
    }

    public boolean isDomDocRoot() {
        return isRoot() && (this._xobj.getDom() instanceof Document);
    }

    public boolean isDomFragRoot() {
        return isRoot() && (this._xobj.getDom() instanceof DocumentFragment);
    }

    public int cchRight() {
        if (!isPositioned()) {
            throw new AssertionError();
        }
        return this._xobj.cchRight(this._pos);
    }

    public int cchLeft() {
        if (!isPositioned()) {
            throw new AssertionError();
        }
        return this._xobj.cchLeft(this._pos);
    }

    /* JADX INFO: Access modifiers changed from: package-private */
    public void createRoot() {
        createDomDocFragRoot();
    }

    /* JADX INFO: Access modifiers changed from: package-private */
    public void createDomDocFragRoot() {
        moveTo(new DocumentFragXobj(this._locale));
    }

    /* JADX INFO: Access modifiers changed from: package-private */
    public void createDomDocumentRoot() {
        moveTo(createDomDocumentRootXobj(this._locale));
    }

    /* JADX INFO: Access modifiers changed from: package-private */
    public void createAttr(QName name) {
        createHelper(new AttrXobj(this._locale, name));
    }

    /* JADX INFO: Access modifiers changed from: package-private */
    public void createComment() {
        createHelper(new CommentXobj(this._locale));
    }

    /* JADX INFO: Access modifiers changed from: package-private */
    public void createProcinst(String target) {
        createHelper(new ProcInstXobj(this._locale, target));
    }

    /* JADX INFO: Access modifiers changed from: package-private */
    public void createElement(QName name) {
        createElement(name, null);
    }

    /* JADX INFO: Access modifiers changed from: package-private */
    public void createElement(QName name, QName parentName) {
        createHelper(createElementXobj(this._locale, name, parentName));
    }

    static Xobj createDomDocumentRootXobj(Locale l) {
        return createDomDocumentRootXobj(l, false);
    }

    /* JADX INFO: Access modifiers changed from: package-private */
    public static Xobj createDomDocumentRootXobj(Locale l, boolean fragment) {
        Xobj xo;
        if (l._saaj == null) {
            if (fragment) {
                xo = new DocumentFragXobj(l);
            } else {
                xo = new DocumentXobj(l);
            }
        } else {
            xo = new SoapPartDocXobj(l);
        }
        if (l._ownerDoc == null) {
            l._ownerDoc = xo.getDom();
        }
        return xo;
    }

    static Xobj createElementXobj(Locale l, QName name, QName parentName) {
        if (l._saaj == null) {
            return new ElementXobj(l, name);
        }
        Class<?> c = l._saaj.identifyElement(name, parentName);
        if (c == SOAPElement.class) {
            return new SoapElementXobj(l, name);
        }
        if (c == SOAPBody.class) {
            return new SoapBodyXobj(l, name);
        }
        if (c == SOAPBodyElement.class) {
            return new SoapBodyElementXobj(l, name);
        }
        if (c == SOAPEnvelope.class) {
            return new SoapEnvelopeXobj(l, name);
        }
        if (c == SOAPHeader.class) {
            return new SoapHeaderXobj(l, name);
        }
        if (c == SOAPHeaderElement.class) {
            return new SoapHeaderElementXobj(l, name);
        }
        if (c == SOAPFaultElement.class) {
            return new SoapFaultElementXobj(l, name);
        }
        if (c == Detail.class) {
            return new DetailXobj(l, name);
        }
        if (c == DetailEntry.class) {
            return new DetailEntryXobj(l, name);
        }
        if (c == SOAPFault.class) {
            return new SoapFaultXobj(l, name);
        }
        throw new IllegalStateException("Unknown SAAJ element class: " + c);
    }

    private void createHelper(Xobj x) {
        if (x._locale != this._locale) {
            throw new AssertionError();
        }
        if (isPositioned()) {
            Cur from = tempCur(x, 0);
            from.moveNode(this);
            from.release();
        }
        moveTo(x);
    }

    /* JADX INFO: Access modifiers changed from: package-private */
    public boolean isSamePos(Cur that) {
        if (isNormal() && (that == null || that.isNormal())) {
            return this._xobj == that._xobj && this._pos == that._pos;
        }
        throw new AssertionError();
    }

    boolean isJustAfterEnd(Cur that) {
        if (!isNormal() || that == null || !that.isNormal() || !that.isNode()) {
            throw new AssertionError();
        }
        return that._xobj.isJustAfterEnd(this._xobj, this._pos);
    }

    boolean isJustAfterEnd(Xobj x) {
        return x.isJustAfterEnd(this._xobj, this._pos);
    }

    boolean isAtEndOf(Cur that) {
        if (that != null && that.isNormal() && that.isNode()) {
            return this._xobj == that._xobj && this._pos == -1;
        }
        throw new AssertionError();
    }

    /* JADX INFO: Access modifiers changed from: package-private */
    public boolean isInSameTree(Cur that) {
        if (!isPositioned() || !that.isPositioned()) {
            throw new AssertionError();
        }
        return this._xobj.isInSameTree(that._xobj);
    }

    /* JADX INFO: Access modifiers changed from: package-private */
    public int comparePosition(Cur that) {
        if (!isPositioned() || !that.isPositioned()) {
            throw new AssertionError();
        }
        if (this._locale != that._locale) {
            return 2;
        }
        Xobj xThis = this._xobj;
        int pThis = this._pos == -1 ? xThis.posAfter() - 1 : this._pos;
        Xobj xThat = that._xobj;
        int pThat = that._pos == -1 ? xThat.posAfter() - 1 : that._pos;
        if (xThis == xThat) {
            return Integer.compare(pThis, pThat);
        }
        int dThis = 0;
        for (Xobj x = xThis._parent; x != null; x = x._parent) {
            dThis++;
            if (x == xThat) {
                return pThat < xThat.posAfter() - 1 ? 1 : -1;
            }
        }
        int dThat = 0;
        for (Xobj x2 = xThat._parent; x2 != null; x2 = x2._parent) {
            dThat++;
            if (x2 == xThis) {
                return pThis < xThis.posAfter() - 1 ? -1 : 1;
            }
        }
        while (dThis > dThat) {
            dThis--;
            if (xThis == null) {
                throw new AssertionError();
            }
            xThis = xThis._parent;
        }
        while (dThat > dThis) {
            dThat--;
            if (xThat == null) {
                throw new AssertionError();
            }
            xThat = xThat._parent;
        }
        if (dThat != dThis) {
            throw new AssertionError();
        }
        if (dThat == 0) {
            return 2;
        }
        if (xThis == null || xThis._parent == null || xThat == null || xThat._parent == null) {
            throw new AssertionError();
        }
        while (xThis._parent != xThat._parent) {
            Xobj xobj = xThis._parent;
            xThis = xobj;
            if (xobj == null) {
                return 2;
            }
            xThat = xThat._parent;
        }
        if (xThis._prevSibling == null || xThat._nextSibling == null) {
            return -1;
        }
        if (xThis._nextSibling == null || xThat._prevSibling == null) {
            return 1;
        }
        while (xThis != null) {
            Xobj xobj2 = xThis._prevSibling;
            xThis = xobj2;
            if (xobj2 == xThat) {
                return 1;
            }
        }
        return -1;
    }

    /* JADX INFO: Access modifiers changed from: package-private */
    public void setName(QName newName) {
        if (!isNode() || newName == null) {
            throw new AssertionError();
        }
        this._xobj.setName(newName);
    }

    /* JADX INFO: Access modifiers changed from: package-private */
    public void moveTo(Xobj x) {
        moveTo(x, 0);
    }

    /* JADX INFO: Access modifiers changed from: package-private */
    public void moveTo(Xobj x, int p) {
        if (x != null && this._locale != x._locale) {
            throw new AssertionError();
        }
        if (x == null && p != -2) {
            throw new AssertionError();
        }
        if (x != null && !x.isNormal(p) && (!x.isVacant() || x._cchValue != 0 || x._user != null)) {
            throw new AssertionError();
        }
        if (this._state != 1 && this._state != 2) {
            throw new AssertionError();
        }
        if (this._state != 2 && this._xobj != null && isOnList(this._xobj._embedded)) {
            throw new AssertionError();
        }
        if (this._state != 1 && (this._xobj == null || !isOnList(this._xobj._embedded))) {
            throw new AssertionError();
        }
        moveToNoCheck(x, p);
        if (isNormal()) {
            return;
        }
        if (!this._xobj.isVacant() || this._xobj._cchValue != 0 || this._xobj._user != null) {
            throw new AssertionError();
        }
    }

    /* JADX INFO: Access modifiers changed from: package-private */
    public void moveToNoCheck(Xobj x, int p) {
        if (this._state == 2 && x != this._xobj) {
            this._xobj._embedded = listRemove(this._xobj._embedded);
            this._locale._registered = listInsert(this._locale._registered);
            this._state = 1;
        }
        this._xobj = x;
        this._pos = p;
    }

    /* JADX INFO: Access modifiers changed from: package-private */
    public void moveToCur(Cur to) {
        if (!isNormal() || (to != null && !to.isNormal())) {
            throw new AssertionError();
        }
        if (to == null) {
            moveTo(null, -2);
        } else {
            moveTo(to._xobj, to._pos);
        }
    }

    /* JADX INFO: Access modifiers changed from: package-private */
    /* JADX WARN: Multi-variable type inference failed */
    public void moveToDom(DomImpl.Dom dom) {
        if (this._locale != dom.locale()) {
            throw new AssertionError();
        }
        if (!(dom instanceof Xobj) && !(dom instanceof SoapPartDom)) {
            throw new AssertionError();
        }
        moveTo(dom instanceof Xobj ? (Xobj) dom : ((SoapPartDom) dom)._docXobj);
    }

    /* JADX INFO: Access modifiers changed from: package-private */
    /* loaded from: classes11.dex */
    public static final class Locations {
        static final /* synthetic */ boolean $assertionsDisabled = false;
        private static final int NULL = -1;
        private static final int _initialSize = 32;
        private int _free;
        private final Locale _locale;
        private int _naked;
        private Xobj[] _xobjs = new Xobj[32];
        private int[] _poses = new int[32];
        private Cur[] _curs = new Cur[32];
        private int[] _next = new int[32];
        private int[] _prev = new int[32];
        private int[] _nextN = new int[32];
        private int[] _prevN = new int[32];

        /* JADX INFO: Access modifiers changed from: package-private */
        public Locations(Locale l) {
            this._locale = l;
            for (int i = 31; i >= 0; i--) {
                if (this._xobjs[i] != null) {
                    throw new AssertionError();
                }
                this._poses[i] = -2;
                this._next[i] = i + 1;
                this._prev[i] = -1;
                this._nextN[i] = -1;
                this._prevN[i] = -1;
            }
            this._next[31] = -1;
            this._free = 0;
            this._naked = -1;
        }

        boolean isSamePos(int i, Cur c) {
            if (this._curs[i] == null) {
                return c._xobj == this._xobjs[i] && c._pos == this._poses[i];
            }
            return c.isSamePos(this._curs[i]);
        }

        boolean isAtEndOf(int i, Cur c) {
            if (this._curs[i] == null && this._poses[i] != 0) {
                throw new AssertionError();
            }
            if (this._curs[i] != null && !this._curs[i].isNode()) {
                throw new AssertionError();
            }
            if (this._curs[i] == null) {
                return c._xobj == this._xobjs[i] && c._pos == -1;
            }
            return c.isAtEndOf(this._curs[i]);
        }

        void moveTo(int i, Cur c) {
            if (this._curs[i] == null) {
                c.moveTo(this._xobjs[i], this._poses[i]);
            } else {
                c.moveToCur(this._curs[i]);
            }
        }

        int insert(int head, int before, int i) {
            return insert(head, before, i, this._next, this._prev);
        }

        int remove(int head, int i) {
            Cur c = this._curs[i];
            if (c == null && this._xobjs[i] == null) {
                throw new AssertionError();
            }
            if (c == null && this._xobjs[i] == null) {
                throw new AssertionError();
            }
            if (c != null) {
                this._curs[i].release();
                this._curs[i] = null;
                if (this._xobjs[i] != null) {
                    throw new AssertionError();
                }
                if (this._poses[i] != -2) {
                    throw new AssertionError();
                }
            } else if (this._xobjs[i] != null && this._poses[i] != -2) {
                this._xobjs[i] = null;
                this._poses[i] = -2;
                this._naked = remove(this._naked, i, this._nextN, this._prevN);
            } else {
                throw new AssertionError();
            }
            int head2 = remove(head, i, this._next, this._prev);
            this._next[i] = this._free;
            this._free = i;
            return head2;
        }

        int allocate(Cur addThis) {
            if (!addThis.isPositioned()) {
                throw new AssertionError();
            }
            if (this._free == -1) {
                makeRoom();
            }
            int i = this._free;
            this._free = this._next[i];
            this._next[i] = -1;
            if (this._prev[i] != -1) {
                throw new AssertionError();
            }
            if (this._curs[i] != null) {
                throw new AssertionError();
            }
            if (this._xobjs[i] != null) {
                throw new AssertionError();
            }
            if (this._poses[i] != -2) {
                throw new AssertionError();
            }
            this._xobjs[i] = addThis._xobj;
            this._poses[i] = addThis._pos;
            this._naked = insert(this._naked, -1, i, this._nextN, this._prevN);
            return i;
        }

        private static int insert(int head, int before, int i, int[] next, int[] prev) {
            if (head == -1) {
                if (before != -1) {
                    throw new AssertionError();
                }
                prev[i] = i;
                return i;
            }
            if (before != -1) {
                prev[i] = prev[before];
                next[i] = before;
                prev[before] = i;
                if (head == before) {
                    return i;
                }
                return head;
            }
            prev[i] = prev[head];
            if (next[i] != -1) {
                throw new AssertionError();
            }
            next[prev[head]] = i;
            prev[head] = i;
            return head;
        }

        private static int remove(int head, int i, int[] next, int[] prev) {
            if (prev[i] == i) {
                if (head != i) {
                    throw new AssertionError();
                }
                head = -1;
            } else {
                if (head == i) {
                    head = next[i];
                } else {
                    next[prev[i]] = next[i];
                }
                if (next[i] == -1) {
                    prev[head] = prev[i];
                } else {
                    prev[next[i]] = prev[i];
                    next[i] = -1;
                }
            }
            prev[i] = -1;
            if (next[i] != -1) {
                throw new AssertionError();
            }
            return head;
        }

        /* JADX INFO: Access modifiers changed from: package-private */
        public void notifyChange() {
            while (true) {
                int i = this._naked;
                if (i != -1) {
                    if (this._curs[i] != null || this._xobjs[i] == null || this._poses[i] == -2) {
                        break;
                    }
                    this._naked = remove(this._naked, i, this._nextN, this._prevN);
                    this._curs[i] = this._locale.getCur();
                    this._curs[i].moveTo(this._xobjs[i], this._poses[i]);
                    this._xobjs[i] = null;
                    this._poses[i] = -2;
                } else {
                    return;
                }
            }
            throw new AssertionError();
        }

        int next(int i) {
            return this._next[i];
        }

        int prev(int i) {
            return this._prev[i];
        }

        private void makeRoom() {
            if (this._free != -1) {
                throw new AssertionError();
            }
            int l = this._xobjs.length;
            Xobj[] oldXobjs = this._xobjs;
            int[] oldPoses = this._poses;
            Cur[] oldCurs = this._curs;
            int[] oldNext = this._next;
            int[] oldPrev = this._prev;
            int[] oldNextN = this._nextN;
            int[] oldPrevN = this._prevN;
            this._xobjs = new Xobj[l * 2];
            this._poses = new int[l * 2];
            this._curs = new Cur[l * 2];
            this._next = new int[l * 2];
            this._prev = new int[l * 2];
            this._nextN = new int[l * 2];
            this._prevN = new int[l * 2];
            System.arraycopy(oldXobjs, 0, this._xobjs, 0, l);
            System.arraycopy(oldPoses, 0, this._poses, 0, l);
            System.arraycopy(oldCurs, 0, this._curs, 0, l);
            System.arraycopy(oldNext, 0, this._next, 0, l);
            System.arraycopy(oldPrev, 0, this._prev, 0, l);
            System.arraycopy(oldNextN, 0, this._nextN, 0, l);
            System.arraycopy(oldPrevN, 0, this._prevN, 0, l);
            for (int i = (l * 2) - 1; i >= l; i--) {
                this._next[i] = i + 1;
                this._prev[i] = -1;
                this._nextN[i] = -1;
                this._prevN[i] = -1;
                this._poses[i] = -2;
            }
            this._next[(l * 2) - 1] = -1;
            this._free = l;
        }
    }

    public void push() {
        if (!isPositioned()) {
            throw new AssertionError();
        }
        int i = this._locale._locations.allocate(this);
        this._stackTop = this._locale._locations.insert(this._stackTop, this._stackTop, i);
    }

    /* JADX INFO: Access modifiers changed from: package-private */
    public void popButStay() {
        if (this._stackTop != -1) {
            this._stackTop = this._locale._locations.remove(this._stackTop, this._stackTop);
        }
    }

    /* JADX INFO: Access modifiers changed from: package-private */
    public boolean pop() {
        if (this._stackTop == -1) {
            return false;
        }
        this._locale._locations.moveTo(this._stackTop, this);
        this._stackTop = this._locale._locations.remove(this._stackTop, this._stackTop);
        return true;
    }

    /* JADX INFO: Access modifiers changed from: package-private */
    public boolean isAtLastPush() {
        if (this._stackTop == -1) {
            throw new AssertionError();
        }
        return this._locale._locations.isSamePos(this._stackTop, this);
    }

    public boolean isAtEndOfLastPush() {
        if (this._stackTop == -1) {
            throw new AssertionError();
        }
        return this._locale._locations.isAtEndOf(this._stackTop, this);
    }

    public void addToSelection(Cur that) {
        if (that == null || !that.isNormal()) {
            throw new AssertionError();
        }
        if (!isPositioned() || !that.isPositioned()) {
            throw new AssertionError();
        }
        int i = this._locale._locations.allocate(that);
        this._selectionFirst = this._locale._locations.insert(this._selectionFirst, -1, i);
        this._selectionCount++;
    }

    public void addToSelection() {
        if (!isPositioned()) {
            throw new AssertionError();
        }
        int i = this._locale._locations.allocate(this);
        this._selectionFirst = this._locale._locations.insert(this._selectionFirst, -1, i);
        this._selectionCount++;
    }

    private int selectionIndex(int i) {
        if (this._selectionN < -1 || i < 0 || i >= this._selectionCount) {
            throw new AssertionError();
        }
        if (this._selectionN == -1) {
            this._selectionN = 0;
            this._selectionLoc = this._selectionFirst;
        }
        while (this._selectionN < i) {
            this._selectionLoc = this._locale._locations.next(this._selectionLoc);
            this._selectionN++;
        }
        while (this._selectionN > i) {
            this._selectionLoc = this._locale._locations.prev(this._selectionLoc);
            this._selectionN--;
        }
        return this._selectionLoc;
    }

    void removeFirstSelection() {
        if (this._selectionCount <= 0) {
            throw new AssertionError();
        }
        int j = selectionIndex(0);
        if (this._selectionN > 0) {
            this._selectionN--;
        } else if (this._selectionN == 0) {
            this._selectionN--;
            this._selectionLoc = -1;
        }
        this._selectionFirst = this._locale._locations.remove(this._selectionFirst, j);
        this._selectionCount--;
    }

    public int selectionCount() {
        return this._selectionCount;
    }

    public void moveToSelection(int i) {
        if (i < 0 || i >= this._selectionCount) {
            throw new AssertionError();
        }
        this._locale._locations.moveTo(selectionIndex(i), this);
    }

    public void clearSelection() {
        if (this._selectionCount < 0) {
            throw new AssertionError();
        }
        while (this._selectionCount > 0) {
            removeFirstSelection();
        }
    }

    public boolean toParent() {
        return toParent(false);
    }

    public boolean toParentRaw() {
        return toParent(true);
    }

    public Xobj getParent() {
        return getParent(false);
    }

    public Xobj getParentRaw() {
        return getParent(true);
    }

    public boolean hasParent() {
        if (!isPositioned()) {
            throw new AssertionError();
        }
        if (this._pos == -1 || (this._pos >= 1 && this._pos < this._xobj.posAfter())) {
            return true;
        }
        if (this._pos == 0 || this._xobj._parent != null) {
            return this._xobj._parent != null;
        }
        throw new AssertionError();
    }

    public Xobj getParentNoRoot() {
        if (!isPositioned()) {
            throw new AssertionError();
        }
        if (this._pos == -1 || (this._pos >= 1 && this._pos < this._xobj.posAfter())) {
            return this._xobj;
        }
        if (this._pos != 0 && this._xobj._parent == null) {
            throw new AssertionError();
        }
        if (this._xobj._parent != null) {
            return this._xobj._parent;
        }
        return null;
    }

    public Xobj getParent(boolean raw) {
        if (!isPositioned()) {
            throw new AssertionError();
        }
        if (this._pos == -1 || (this._pos >= 1 && this._pos < this._xobj.posAfter())) {
            return this._xobj;
        }
        if (this._pos != 0 && this._xobj._parent == null) {
            throw new AssertionError();
        }
        if (this._xobj._parent != null) {
            return this._xobj._parent;
        }
        if (raw || this._xobj.isRoot()) {
            return null;
        }
        Cur r = this._locale.tempCur();
        r.createRoot();
        Xobj root = r._xobj;
        r.next();
        moveNode(r);
        r.release();
        return root;
    }

    public boolean toParent(boolean raw) {
        Xobj parent = getParent(raw);
        if (parent == null) {
            return false;
        }
        moveTo(parent);
        return true;
    }

    public void toRoot() {
        Xobj xobj = this._xobj;
        while (true) {
            if (xobj.isRoot()) {
                break;
            }
            if (xobj._parent == null) {
                Cur r = this._locale.tempCur();
                r.createRoot();
                Xobj root = r._xobj;
                r.next();
                moveNode(r);
                r.release();
                xobj = root;
                break;
            }
            xobj = xobj._parent;
        }
        moveTo(xobj);
    }

    public boolean hasText() {
        if (!isNode()) {
            throw new AssertionError();
        }
        return this._xobj.hasTextEnsureOccupancy();
    }

    public boolean hasAttrs() {
        if (!isNode()) {
            throw new AssertionError();
        }
        return this._xobj.hasAttrs();
    }

    public boolean hasChildren() {
        if (!isNode()) {
            throw new AssertionError();
        }
        return this._xobj.hasChildren();
    }

    public boolean toFirstChild() {
        if (!isNode()) {
            throw new AssertionError();
        }
        if (!this._xobj.hasChildren()) {
            return false;
        }
        Xobj x = this._xobj._firstChild;
        while (x.isAttr()) {
            x = x._nextSibling;
        }
        moveTo(x);
        return true;
    }

    public boolean toLastChild() {
        if (!isNode()) {
            throw new AssertionError();
        }
        if (!this._xobj.hasChildren()) {
            return false;
        }
        moveTo(this._xobj._lastChild);
        return true;
    }

    public boolean toNextSibling() {
        if (!isNode()) {
            throw new AssertionError();
        }
        if (this._xobj.isAttr()) {
            if (this._xobj._nextSibling != null && this._xobj._nextSibling.isAttr()) {
                moveTo(this._xobj._nextSibling);
                return true;
            }
            return false;
        }
        if (this._xobj._nextSibling != null) {
            moveTo(this._xobj._nextSibling);
            return true;
        }
        return false;
    }

    public void setValueAsQName(QName qname) {
        if (!isNode()) {
            throw new AssertionError();
        }
        String value = qname.getLocalPart();
        String ns = qname.getNamespaceURI();
        String prefix = prefixForNamespace(ns, qname.getPrefix().length() > 0 ? qname.getPrefix() : null, true);
        if (prefix.length() > 0) {
            value = prefix + ":" + value;
        }
        setValue(value);
    }

    public void setValue(String value) {
        if (!isNode()) {
            throw new AssertionError();
        }
        moveNodeContents(null, false);
        next();
        insertString(value);
        toParent();
    }

    public void removeFollowingAttrs() {
        if (!isAttr()) {
            throw new AssertionError();
        }
        QName attrName = getName();
        push();
        if (toNextAttr()) {
            while (isAttr()) {
                if (getName().equals(attrName)) {
                    moveNode(null);
                } else if (!toNextAttr()) {
                    break;
                }
            }
        }
        pop();
    }

    public String getAttrValue(QName name) {
        String s = null;
        push();
        if (toAttr(name)) {
            s = getValueAsString();
        }
        pop();
        return s;
    }

    public void setAttrValueAsQName(QName value) {
        if (!isContainer()) {
            throw new AssertionError();
        }
        QName name = Locale._xsiType;
        if (value == null) {
            this._xobj.removeAttr(name);
            return;
        }
        if (toAttr(name)) {
            removeFollowingAttrs();
        } else {
            next();
            createAttr(name);
        }
        setValueAsQName(value);
        toParent();
    }

    public boolean removeAttr(QName name) {
        if (!isContainer()) {
            throw new AssertionError();
        }
        return this._xobj.removeAttr(name);
    }

    public void setAttrValue(QName name, String value) {
        if (!isContainer()) {
            throw new AssertionError();
        }
        this._xobj.setAttr(name, value);
    }

    public boolean toAttr(QName name) {
        if (!isNode()) {
            throw new AssertionError();
        }
        Xobj a = this._xobj.getAttr(name);
        if (a == null) {
            return false;
        }
        moveTo(a);
        return true;
    }

    public boolean toFirstAttr() {
        if (!isNode()) {
            throw new AssertionError();
        }
        Xobj firstAttr = this._xobj.firstAttr();
        if (firstAttr == null) {
            return false;
        }
        moveTo(firstAttr);
        return true;
    }

    public boolean toLastAttr() {
        if (!isNode()) {
            throw new AssertionError();
        }
        if (!toFirstAttr()) {
            return false;
        }
        do {
        } while (toNextAttr());
        return true;
    }

    public boolean toNextAttr() {
        if (!isAttr() && !isContainer()) {
            throw new AssertionError();
        }
        Xobj nextAttr = this._xobj.nextAttr();
        if (nextAttr == null) {
            return false;
        }
        moveTo(nextAttr);
        return true;
    }

    public boolean toPrevAttr() {
        if (isAttr()) {
            if (this._xobj._prevSibling == null) {
                moveTo(this._xobj.ensureParent());
                return true;
            }
            moveTo(this._xobj._prevSibling);
            return true;
        }
        prev();
        if (!isContainer()) {
            next();
            return false;
        }
        return toLastAttr();
    }

    public boolean skipWithAttrs() {
        if (!isNode()) {
            throw new AssertionError();
        }
        if (skip()) {
            return true;
        }
        if (this._xobj.isRoot()) {
            return false;
        }
        if (!this._xobj.isAttr()) {
            throw new AssertionError();
        }
        toParent();
        next();
        return true;
    }

    public boolean skip() {
        if (!isNode()) {
            throw new AssertionError();
        }
        if (this._xobj.isRoot()) {
            return false;
        }
        if (this._xobj.isAttr()) {
            if (this._xobj._nextSibling == null || !this._xobj._nextSibling.isAttr()) {
                return false;
            }
            moveTo(this._xobj._nextSibling, 0);
            return true;
        }
        moveTo(getNormal(this._xobj, this._xobj.posAfter()), this._posTemp);
        return true;
    }

    public void toEnd() {
        if (!isNode()) {
            throw new AssertionError();
        }
        moveTo(this._xobj, -1);
    }

    public void moveToCharNode(CharNode node) {
        if (node.getDom() == null || node.getDom().locale() != this._locale) {
            throw new AssertionError();
        }
        moveToDom(node.getDom());
        this._xobj.ensureOccupancy();
        Xobj xobj = this._xobj;
        CharNode n = updateCharNodes(this._locale, this._xobj, this._xobj._charNodesValue, this._xobj._cchValue);
        xobj._charNodesValue = n;
        while (n != null) {
            if (node != n) {
                n = n._next;
            } else {
                moveTo(getNormal(this._xobj, n._off + 1), this._posTemp);
                return;
            }
        }
        Xobj xobj2 = this._xobj;
        CharNode n2 = updateCharNodes(this._locale, this._xobj, this._xobj._charNodesAfter, this._xobj._cchAfter);
        xobj2._charNodesAfter = n2;
        while (n2 != null) {
            if (node != n2) {
                n2 = n2._next;
            } else {
                moveTo(getNormal(this._xobj, n2._off + this._xobj._cchValue + 2), this._posTemp);
                return;
            }
        }
        throw new AssertionError();
    }

    public boolean prevWithAttrs() {
        if (prev()) {
            return true;
        }
        if (!isAttr()) {
            return false;
        }
        toParent();
        return true;
    }

    public boolean prev() {
        if (!isPositioned()) {
            throw new AssertionError();
        }
        int p = 0;
        if (this._xobj.isRoot() && this._pos == 0) {
            return false;
        }
        if (this._xobj.isAttr() && this._pos == 0 && this._xobj._prevSibling == null) {
            return false;
        }
        Xobj x = getDenormal();
        int p2 = this._posTemp;
        if (p2 <= 0) {
            throw new AssertionError();
        }
        int pa = x.posAfter();
        if (p2 > pa) {
            p = pa;
        } else if (p2 == pa) {
            if (x.isAttr() && (x._cchAfter > 0 || x._nextSibling == null || !x._nextSibling.isAttr())) {
                x = x.ensureParent();
                p = 0;
            } else {
                p = -1;
            }
        } else if (p2 == pa - 1) {
            x.ensureOccupancy();
            if (x._cchValue > 0) {
                p = 1;
            }
        } else if (p2 > 1) {
            p = 1;
        } else {
            if (p2 != 1) {
                throw new AssertionError();
            }
            p = 0;
        }
        moveTo(getNormal(x, p), this._posTemp);
        return true;
    }

    public boolean next(boolean withAttrs) {
        return withAttrs ? nextWithAttrs() : next();
    }

    public boolean nextWithAttrs() {
        int k = kind();
        if (kindIsContainer(k)) {
            if (toFirstAttr()) {
                return true;
            }
        } else if (k == -3) {
            if (next()) {
                return true;
            }
            toParent();
            if (!toParentRaw()) {
                return false;
            }
        }
        return next();
    }

    public boolean next() {
        int p;
        if (!isNormal()) {
            throw new AssertionError();
        }
        Xobj x = this._xobj;
        int p2 = this._pos;
        int pa = x.posAfter();
        if (p2 >= pa) {
            p = this._xobj.posMax();
        } else if (p2 == -1) {
            if (x.isRoot()) {
                return false;
            }
            if (x.isAttr() && (x._nextSibling == null || !x._nextSibling.isAttr())) {
                return false;
            }
            p = pa;
        } else if (p2 > 0) {
            if (x._firstChild != null && x._firstChild.isAttr()) {
                throw new AssertionError();
            }
            if (x._firstChild != null) {
                x = x._firstChild;
                p = 0;
            } else {
                p = -1;
            }
        } else {
            if (p2 != 0) {
                throw new AssertionError();
            }
            x.ensureOccupancy();
            p = 1;
            if (x._cchValue == 0 && x._firstChild != null) {
                if (x._firstChild.isAttr()) {
                    Xobj a = x._firstChild;
                    while (a._nextSibling != null && a._nextSibling.isAttr()) {
                        a = a._nextSibling;
                    }
                    if (a._cchAfter > 0) {
                        x = a;
                        p = a.posAfter();
                    } else if (a._nextSibling != null) {
                        x = a._nextSibling;
                        p = 0;
                    }
                } else {
                    x = x._firstChild;
                    p = 0;
                }
            }
        }
        moveTo(getNormal(x, p), this._posTemp);
        return true;
    }

    /* JADX INFO: Access modifiers changed from: package-private */
    public int prevChars(int cch) {
        if (!isPositioned()) {
            throw new AssertionError();
        }
        int cchLeft = cchLeft();
        if (cch < 0 || cch > cchLeft) {
            cch = cchLeft;
        }
        if (cch != 0) {
            moveTo(getNormal(getDenormal(), this._posTemp - cch), this._posTemp);
        }
        return cch;
    }

    /* JADX INFO: Access modifiers changed from: package-private */
    public int nextChars(int cch) {
        if (!isPositioned()) {
            throw new AssertionError();
        }
        int cchRight = cchRight();
        if (cchRight == 0) {
            return 0;
        }
        if (cch < 0 || cch >= cchRight) {
            next();
            return cchRight;
        }
        moveTo(getNormal(this._xobj, this._pos + cch), this._posTemp);
        return cch;
    }

    /* JADX INFO: Access modifiers changed from: package-private */
    /* JADX WARN: Multi-variable type inference failed */
    public void setCharNodes(CharNode nodes) {
        if (nodes != null && this._locale != nodes.locale()) {
            throw new AssertionError();
        }
        if (!isPositioned()) {
            throw new AssertionError();
        }
        Xobj denormal = getDenormal();
        int p = this._posTemp;
        if (denormal.isRoot() && (p <= 0 || p >= denormal.posAfter())) {
            throw new AssertionError();
        }
        if (p >= denormal.posAfter()) {
            denormal._charNodesAfter = nodes;
        } else {
            denormal._charNodesValue = nodes;
        }
        while (nodes != null) {
            nodes.setDom((DomImpl.Dom) denormal);
            nodes = nodes._next;
        }
    }

    /* JADX INFO: Access modifiers changed from: package-private */
    public CharNode getCharNodes() {
        if (!isPositioned()) {
            throw new AssertionError();
        }
        if (isRoot()) {
            throw new AssertionError();
        }
        Xobj x = getDenormal();
        if (this._posTemp >= x.posAfter()) {
            CharNode nodes = updateCharNodes(this._locale, x, x._charNodesAfter, x._cchAfter);
            x._charNodesAfter = nodes;
            return nodes;
        }
        x.ensureOccupancy();
        CharNode nodes2 = updateCharNodes(this._locale, x, x._charNodesValue, x._cchValue);
        x._charNodesValue = nodes2;
        return nodes2;
    }

    /* JADX INFO: Access modifiers changed from: package-private */
    /* JADX WARN: Multi-variable type inference failed */
    public static CharNode updateCharNodes(Locale l, Xobj xobj, CharNode nodes, int cch) {
        if (nodes != null && nodes.locale() != l) {
            throw new AssertionError();
        }
        CharNode node = nodes;
        int i = 0;
        while (node != null && cch > 0) {
            if (node.getDom() != xobj) {
                throw new AssertionError();
            }
            if (node._cch > cch) {
                node._cch = cch;
            }
            node._off = i;
            i += node._cch;
            cch -= node._cch;
            node = node._next;
        }
        if (cch <= 0) {
            while (node != null) {
                if (node.getDom() != xobj) {
                    throw new AssertionError();
                }
                if (node._cch != 0) {
                    node._cch = 0;
                }
                node._off = i;
                node = node._next;
            }
            return nodes;
        }
        CharNode node2 = l.createTextNode();
        node2.setDom((DomImpl.Dom) xobj);
        node2._cch = cch;
        node2._off = i;
        return CharNode.appendNode(nodes, node2);
    }

    /* JADX INFO: Access modifiers changed from: package-private */
    public final QName getXsiTypeName() {
        if (!isNode()) {
            throw new AssertionError();
        }
        return this._xobj.getXsiTypeName();
    }

    /* JADX INFO: Access modifiers changed from: package-private */
    public final void setXsiType(QName value) {
        if (!isContainer()) {
            throw new AssertionError();
        }
        setAttrValueAsQName(value);
    }

    /* JADX INFO: Access modifiers changed from: package-private */
    public final String namespaceForPrefix(String prefix, boolean defaultAlwaysMapped) {
        return this._xobj.namespaceForPrefix(prefix, defaultAlwaysMapped);
    }

    /* JADX INFO: Access modifiers changed from: package-private */
    public final String prefixForNamespace(String ns, String suggestion, boolean createIfMissing) {
        return (isContainer() ? this._xobj : getParent()).prefixForNamespace(ns, suggestion, createIfMissing);
    }

    /* JADX INFO: Access modifiers changed from: package-private */
    public boolean contains(Cur that) {
        if (!isNode()) {
            throw new AssertionError();
        }
        if (that == null || !that.isPositioned()) {
            throw new AssertionError();
        }
        return this._xobj.contains(that);
    }

    /* JADX INFO: Access modifiers changed from: package-private */
    public void insertString(String s) {
        if (s != null) {
            insertChars(s, 0, s.length());
        }
    }

    /* JADX INFO: Access modifiers changed from: package-private */
    public void insertChars(Object src, int off, int cch) {
        if (!isPositioned() || isRoot()) {
            throw new AssertionError();
        }
        if (!CharUtil.isValid(src, off, cch)) {
            throw new AssertionError();
        }
        if (cch <= 0) {
            return;
        }
        this._locale.notifyChange();
        if (this._pos == -1) {
            this._xobj.ensureOccupancy();
        }
        Xobj x = getDenormal();
        int p = this._posTemp;
        if (p <= 0) {
            throw new AssertionError();
        }
        x.insertCharsHelper(p, src, off, cch, true);
        moveTo(x, p);
        this._locale._versionAll++;
    }

    /* JADX INFO: Access modifiers changed from: package-private */
    public Object moveChars(Cur to, int cchMove) {
        int cchMove2;
        if (!isPositioned()) {
            throw new AssertionError();
        }
        if (cchMove > 0 && cchMove > cchRight()) {
            throw new AssertionError();
        }
        if (to != null && (!to.isPositioned() || to.isRoot())) {
            throw new AssertionError();
        }
        if (cchMove >= 0) {
            cchMove2 = cchMove;
        } else {
            cchMove2 = cchRight();
        }
        if (cchMove2 == 0) {
            this._offSrc = 0;
            this._cchSrc = 0;
            return null;
        }
        Object srcMoved = getChars(cchMove2);
        int offMoved = this._offSrc;
        if (isText()) {
            if ((this._pos >= this._xobj.posAfter() ? this._xobj._parent : this._xobj).isOccupied()) {
                if (to == null) {
                    for (Bookmark b = this._xobj._bookmarks; b != null; b = b._next) {
                        if (inChars(b, cchMove2, false)) {
                            Cur c = this._locale.tempCur();
                            c.createRoot();
                            c.next();
                            Object chars = moveChars(c, cchMove2);
                            c.release();
                            return chars;
                        }
                    }
                } else {
                    if (inChars(to, cchMove2, true)) {
                        to.moveToCur(this);
                        nextChars(cchMove2);
                        this._offSrc = offMoved;
                        this._cchSrc = cchMove2;
                        return srcMoved;
                    }
                    to.insertChars(srcMoved, offMoved, cchMove2);
                }
                this._locale.notifyChange();
                if (to == null) {
                    this._xobj.removeCharsHelper(this._pos, cchMove2, null, -2, false, true);
                } else {
                    this._xobj.removeCharsHelper(this._pos, cchMove2, to._xobj, to._pos, false, true);
                }
                this._locale._versionAll++;
                this._offSrc = offMoved;
                this._cchSrc = cchMove2;
                return srcMoved;
            }
        }
        throw new AssertionError();
    }

    /* JADX INFO: Access modifiers changed from: package-private */
    public void moveNode(Cur to) {
        if (!isNode() || isRoot()) {
            throw new AssertionError();
        }
        if (to != null && !to.isPositioned()) {
            throw new AssertionError();
        }
        if (to != null && contains(to)) {
            throw new AssertionError();
        }
        if (to != null && to.isRoot()) {
            throw new AssertionError();
        }
        Xobj x = this._xobj;
        skip();
        moveNode(x, to);
    }

    private static void transferChars(Xobj xFrom, int pFrom, Xobj xTo, int pTo, int cch) {
        if (xFrom == xTo) {
            throw new AssertionError();
        }
        if (xFrom._locale != xTo._locale) {
            throw new AssertionError();
        }
        if (pFrom <= 0 || pFrom >= xFrom.posMax()) {
            throw new AssertionError();
        }
        if (pTo <= 0 || pTo > xTo.posMax()) {
            throw new AssertionError();
        }
        if (cch <= 0 || cch > xFrom.cchRight(pFrom)) {
            throw new AssertionError();
        }
        if (pTo < xTo.posAfter() && !xTo.isOccupied()) {
            throw new AssertionError();
        }
        xTo.insertCharsHelper(pTo, xFrom.getCharsHelper(pFrom, cch), xFrom._locale._offSrc, xFrom._locale._cchSrc, false);
        xFrom.removeCharsHelper(pFrom, cch, xTo, pTo, true, false);
    }

    /* JADX INFO: Access modifiers changed from: package-private */
    public static void moveNode(Xobj x, Cur to) {
        if (x == null || x.isRoot()) {
            throw new AssertionError();
        }
        if (to != null && !to.isPositioned()) {
            throw new AssertionError();
        }
        if (to != null && x.contains(to)) {
            throw new AssertionError();
        }
        if (to != null && to.isRoot()) {
            throw new AssertionError();
        }
        if (to != null) {
            if (to._pos == -1) {
                to._xobj.ensureOccupancy();
            }
            if ((to._pos == 0 && to._xobj == x) || to.isJustAfterEnd(x)) {
                to.moveTo(x);
                return;
            }
        }
        x._locale.notifyChange();
        x._locale._versionAll++;
        x._locale._versionSansText++;
        if (to != null && to._locale != x._locale) {
            to._locale.notifyChange();
            to._locale._versionAll++;
            to._locale._versionSansText++;
        }
        if (x.isAttr()) {
            x.invalidateSpecialAttr(to == null ? null : to.getParentRaw());
        } else {
            if (x._parent != null) {
                x._parent.invalidateUser();
            }
            if (to != null && to.hasParent()) {
                to.getParent().invalidateUser();
            }
        }
        if (x._cchAfter > 0) {
            transferChars(x, x.posAfter(), x.getDenormal(0), x.posTemp(), x._cchAfter);
        }
        if (x._cchAfter != 0) {
            throw new AssertionError();
        }
        x._locale.embedCurs();
        Xobj y = x;
        while (y != null) {
            while (y._embedded != null) {
                y._embedded.moveTo(x.getNormal(x.posAfter()));
            }
            y.disconnectUser();
            if (to != null) {
                y._locale = to._locale;
            }
            y = y.walk(x, true);
        }
        x.removeXobj();
        if (to != null) {
            Xobj here = to._xobj;
            boolean append = to._pos != 0;
            int cchRight = to.cchRight();
            if (cchRight > 0) {
                to.push();
                to.next();
                here = to._xobj;
                append = to._pos != 0;
                to.pop();
            }
            if (append) {
                here.appendXobj(x);
            } else {
                here.insertXobj(x);
            }
            if (cchRight > 0) {
                transferChars(to._xobj, to._pos, x, x.posAfter(), cchRight);
            }
            to.moveTo(x);
        }
    }

    /* JADX INFO: Access modifiers changed from: package-private */
    public void moveNodeContents(Cur to, boolean moveAttrs) {
        if (this._pos != 0) {
            throw new AssertionError();
        }
        if (to != null && to.isRoot()) {
            throw new AssertionError();
        }
        moveNodeContents(this._xobj, to, moveAttrs);
    }

    /* JADX INFO: Access modifiers changed from: package-private */
    public static void moveNodeContents(Xobj x, Cur to, boolean moveAttrs) {
        int p;
        int cch;
        Cur to2 = to;
        if (to2 != null && to2.isRoot()) {
            throw new AssertionError();
        }
        boolean hasAttrs = x.hasAttrs();
        boolean noSubNodesToMove = (x.hasChildren() || (moveAttrs && hasAttrs)) ? false : true;
        if (noSubNodesToMove) {
            if (x.isVacant() && to2 == null) {
                x.clearBit(256);
                x.invalidateUser();
                x.invalidateSpecialAttr(null);
                x._locale._versionAll++;
                return;
            }
            if (x.hasTextEnsureOccupancy()) {
                Cur c = x.tempCur();
                c.next();
                c.moveChars(to2, -1);
                c.release();
                return;
            }
            return;
        }
        if (to2 != null) {
            if (x == to2._xobj && to2._pos == -1) {
                to2.moveTo(x);
                if (moveAttrs && hasAttrs) {
                    r4 = true;
                }
                to2.next(r4);
                return;
            }
            boolean isAtLeftEdge = false;
            if (to2._locale == x._locale) {
                to2.push();
                to2.moveTo(x);
                to2.next(moveAttrs && hasAttrs);
                isAtLeftEdge = to2.isAtLastPush();
                to2.pop();
            }
            if (isAtLeftEdge) {
                return;
            }
            if (x.contains(to)) {
                throw new AssertionError();
            }
            if (!to2.getParent().isOccupied()) {
                throw new AssertionError();
            }
        }
        int valueMovedCch = 0;
        if (x.hasTextNoEnsureOccupancy()) {
            Cur c2 = x.tempCur();
            c2.next();
            c2.moveChars(to2, -1);
            c2.release();
            if (to2 != null) {
                int i = c2._cchSrc;
                valueMovedCch = i;
                to2.nextChars(i);
            }
        }
        x._locale.embedCurs();
        Xobj firstToMove = x.walk(x, true);
        boolean sawBookmark = false;
        Xobj y = firstToMove;
        while (y != null) {
            if (y._parent == x && y.isAttr()) {
                if (y._cchAfter != 0) {
                    throw new AssertionError();
                }
                if (!moveAttrs) {
                    firstToMove = y._nextSibling;
                    y = y.walk(x, true);
                } else {
                    y.invalidateSpecialAttr(to2 == null ? null : to2.getParent());
                }
            }
            while (true) {
                Cur c3 = y._embedded;
                if (c3 == null) {
                    break;
                } else {
                    c3.moveTo(x, -1);
                }
            }
            y.disconnectUser();
            if (to2 != null) {
                y._locale = to2._locale;
            }
            sawBookmark = sawBookmark || y._bookmarks != null;
            y = y.walk(x, true);
        }
        Xobj lastToMove = x._lastChild;
        Cur surragateTo = null;
        if (sawBookmark && to2 == null) {
            Cur tempCur = x._locale.tempCur();
            to2 = tempCur;
            surragateTo = tempCur;
            to2.createRoot();
            to2.next();
        }
        if (!lastToMove.isAttr()) {
            x.invalidateUser();
        }
        x._locale._versionAll++;
        x._locale._versionSansText++;
        if (to2 != null && valueMovedCch == 0) {
            to2.getParent().invalidateUser();
            to2._locale._versionAll++;
            to2._locale._versionSansText++;
        }
        x.removeXobjs(firstToMove, lastToMove);
        if (to2 != null) {
            Xobj here = to2._xobj;
            boolean append = to2._pos != 0;
            int cchRight = to2.cchRight();
            if (cchRight > 0) {
                to2.push();
                to2.next();
                here = to2._xobj;
                append = to2._pos != 0;
                to2.pop();
            }
            if (firstToMove.isAttr()) {
                Xobj lastNewAttr = firstToMove;
                while (lastNewAttr._nextSibling != null && lastNewAttr._nextSibling.isAttr()) {
                    lastNewAttr = lastNewAttr._nextSibling;
                }
                Xobj y2 = to2.getParent();
                if (cchRight > 0) {
                    transferChars(to2._xobj, to2._pos, lastNewAttr, lastNewAttr.posMax(), cchRight);
                }
                if (y2.hasTextNoEnsureOccupancy()) {
                    if (y2._cchValue > 0) {
                        p = 1;
                        cch = y2._cchValue;
                    } else {
                        y2 = y2.lastAttr();
                        if (y2 == null) {
                            throw new AssertionError();
                        }
                        p = y2.posAfter();
                        cch = y2._cchAfter;
                    }
                    transferChars(y2, p, lastNewAttr, lastNewAttr.posAfter(), cch);
                }
            } else if (cchRight > 0) {
                transferChars(to2._xobj, to2._pos, lastToMove, lastToMove.posMax(), cchRight);
            }
            if (append) {
                here.appendXobjs(firstToMove, lastToMove);
            } else {
                here.insertXobjs(firstToMove, lastToMove);
            }
            to2.moveTo(firstToMove);
            to2.prevChars(valueMovedCch);
        }
        if (surragateTo != null) {
            surragateTo.release();
        }
    }

    /* JADX INFO: Access modifiers changed from: protected */
    public final Bookmark setBookmark(Object key, Object value) {
        if (!isNormal()) {
            throw new AssertionError();
        }
        if (key == null) {
            throw new AssertionError();
        }
        return this._xobj.setBookmark(this._pos, key, value);
    }

    /* JADX INFO: Access modifiers changed from: package-private */
    public Object getBookmark(Object key) {
        if (!isNormal()) {
            throw new AssertionError();
        }
        if (key == null) {
            throw new AssertionError();
        }
        for (Bookmark b = this._xobj._bookmarks; b != null; b = b._next) {
            if (b._pos == this._pos && b._key == key) {
                return b._value;
            }
        }
        return null;
    }

    /* JADX INFO: Access modifiers changed from: package-private */
    public int firstBookmarkInChars(Object key, int cch) {
        if (!isNormal()) {
            throw new AssertionError();
        }
        if (key == null) {
            throw new AssertionError();
        }
        if (cch <= 0) {
            throw new AssertionError();
        }
        if (cch > cchRight()) {
            throw new AssertionError();
        }
        int d = -1;
        if (isText()) {
            for (Bookmark b = this._xobj._bookmarks; b != null; b = b._next) {
                if (b._key == key && inChars(b, cch, false)) {
                    d = (d == -1 || b._pos - this._pos < d) ? b._pos - this._pos : d;
                }
            }
        }
        return d;
    }

    /* JADX INFO: Access modifiers changed from: package-private */
    public int firstBookmarkInCharsLeft(Object key, int cch) {
        int cch2;
        if (!isNormal()) {
            throw new AssertionError();
        }
        if (key == null) {
            throw new AssertionError();
        }
        if (cch <= 0) {
            throw new AssertionError();
        }
        if (cch > cchLeft()) {
            throw new AssertionError();
        }
        int d = -1;
        if (cchLeft() > 0) {
            Xobj x = getDenormal();
            int p = this._posTemp - cch;
            Bookmark b = x._bookmarks;
            while (b != null) {
                if (b._key == key) {
                    cch2 = cch;
                    if (x.inChars(p, b._xobj, b._pos, cch2, false)) {
                        d = (d == -1 || b._pos - p < d) ? b._pos - p : d;
                    }
                } else {
                    cch2 = cch;
                }
                b = b._next;
                cch = cch2;
            }
        }
        return d;
    }

    /* JADX INFO: Access modifiers changed from: package-private */
    public String getCharsAsString() {
        if (!isNormal() || this._xobj == null) {
            throw new AssertionError();
        }
        return getCharsAsString(1);
    }

    /* JADX INFO: Access modifiers changed from: package-private */
    public String getCharsAsString(int wsr) {
        return this._xobj.getCharsAsString(this._pos, -1, wsr);
    }

    /* JADX INFO: Access modifiers changed from: package-private */
    public String getValueAsString(int wsr) {
        if (!isNode()) {
            throw new AssertionError();
        }
        return this._xobj.getValueAsString(wsr);
    }

    /* JADX INFO: Access modifiers changed from: package-private */
    public String getValueAsString() {
        if (!isNode()) {
            throw new AssertionError();
        }
        if (hasChildren()) {
            throw new AssertionError();
        }
        return this._xobj.getValueAsString();
    }

    /* JADX INFO: Access modifiers changed from: package-private */
    public Object getChars(int cch) {
        if (!isPositioned()) {
            throw new AssertionError();
        }
        return this._xobj.getChars(this._pos, cch, this);
    }

    /* JADX INFO: Access modifiers changed from: package-private */
    public Object getFirstChars() {
        if (!isNode()) {
            throw new AssertionError();
        }
        Object src = this._xobj.getFirstChars();
        this._offSrc = this._locale._offSrc;
        this._cchSrc = this._locale._cchSrc;
        return src;
    }

    /* JADX INFO: Access modifiers changed from: package-private */
    public void copyNode(Cur to) {
        if (to == null) {
            throw new AssertionError();
        }
        if (!isNode()) {
            throw new AssertionError();
        }
        Xobj copy = this._xobj.copyNode(to._locale);
        if (to.isPositioned()) {
            moveNode(copy, to);
        } else {
            to.moveTo(copy);
        }
    }

    public Cur weakCur(Object o) {
        Cur c = this._locale.weakCur(o);
        c.moveToCur(this);
        return c;
    }

    /* JADX INFO: Access modifiers changed from: package-private */
    public Cur tempCur() {
        Cur c = this._locale.tempCur(null);
        c.moveToCur(this);
        return c;
    }

    private Cur tempCur(Xobj x, int p) {
        if (x != null && this._locale != x._locale) {
            throw new AssertionError();
        }
        if (x == null && p != -2) {
            throw new AssertionError();
        }
        Cur c = this._locale.tempCur();
        if (x != null) {
            c.moveTo(getNormal(x, p), this._posTemp);
        }
        return c;
    }

    /* JADX INFO: Access modifiers changed from: package-private */
    public boolean inChars(Cur c, int cch, boolean includeEnd) {
        if (!isPositioned() || !isText() || cchRight() < cch) {
            throw new AssertionError();
        }
        if (c.isNormal()) {
            return this._xobj.inChars(this._pos, c._xobj, c._pos, cch, includeEnd);
        }
        throw new AssertionError();
    }

    boolean inChars(Bookmark b, int cch, boolean includeEnd) {
        if (!isPositioned() || !isText() || cchRight() < cch) {
            throw new AssertionError();
        }
        if (b._xobj.isNormal(b._pos)) {
            return this._xobj.inChars(this._pos, b._xobj, b._pos, cch, includeEnd);
        }
        throw new AssertionError();
    }

    private Xobj getNormal(Xobj x, int p) {
        Xobj nx = x.getNormal(p);
        this._posTemp = x._locale._posTemp;
        return nx;
    }

    private Xobj getDenormal() {
        if (!isPositioned()) {
            throw new AssertionError();
        }
        return getDenormal(this._xobj, this._pos);
    }

    private Xobj getDenormal(Xobj x, int p) {
        Xobj dx = x.getDenormal(p);
        this._posTemp = x._locale._posTemp;
        return dx;
    }

    /* JADX INFO: Access modifiers changed from: package-private */
    public void setType(SchemaType type) {
        setType(type, true);
    }

    /* JADX INFO: Access modifiers changed from: package-private */
    public void setType(SchemaType type, boolean complain) {
        if (type == null) {
            throw new AssertionError();
        }
        if (!isUserNode()) {
            throw new AssertionError();
        }
        TypeStoreUser user = peekUser();
        if (user != null && user.get_schema_type() == type) {
            return;
        }
        if (isRoot()) {
            this._xobj.setStableType(type);
            return;
        }
        TypeStoreUser parentUser = this._xobj.ensureParent().getUser();
        if (isAttr()) {
            if (complain && parentUser.get_attribute_type(getName()) != type) {
                throw new IllegalArgumentException("Can't set type of attribute to " + type.toString());
            }
            return;
        }
        if (!isElem()) {
            throw new AssertionError();
        }
        if (parentUser.get_element_type(getName(), null) == type) {
            removeAttr(Locale._xsiType);
            return;
        }
        QName typeName = type.getName();
        if (typeName == null) {
            if (complain) {
                throw new IllegalArgumentException("Can't set type of element, type is un-named");
            }
        } else if (parentUser.get_element_type(getName(), typeName) != type) {
            if (complain) {
                throw new IllegalArgumentException("Can't set type of element, invalid type");
            }
        } else {
            setAttrValueAsQName(typeName);
        }
    }

    /* JADX INFO: Access modifiers changed from: package-private */
    public void setSubstitution(QName name, SchemaType type) {
        if (name == null) {
            throw new AssertionError();
        }
        if (type == null) {
            throw new AssertionError();
        }
        if (!isUserNode()) {
            throw new AssertionError();
        }
        TypeStoreUser user = peekUser();
        if ((user != null && user.get_schema_type() == type && name.equals(getName())) || isRoot()) {
            return;
        }
        TypeStoreUser parentUser = this._xobj.ensureParent().getUser();
        if (isAttr()) {
            return;
        }
        if (!isElem()) {
            throw new AssertionError();
        }
        if (parentUser.get_element_type(name, null) == type) {
            setName(name);
            removeAttr(Locale._xsiType);
            return;
        }
        QName typeName = type.getName();
        if (typeName == null || parentUser.get_element_type(name, typeName) != type) {
            return;
        }
        setName(name);
        setAttrValueAsQName(typeName);
    }

    TypeStoreUser peekUser() {
        if (!isUserNode()) {
            throw new AssertionError();
        }
        return this._xobj._user;
    }

    public XmlObject getObject() {
        if (isUserNode()) {
            return (XmlObject) getUser();
        }
        return null;
    }

    /* JADX INFO: Access modifiers changed from: package-private */
    public TypeStoreUser getUser() {
        if (!isUserNode()) {
            throw new AssertionError();
        }
        return this._xobj.getUser();
    }

    public DomImpl.Dom getDom() {
        if (!isNormal()) {
            throw new AssertionError();
        }
        if (!isPositioned()) {
            throw new AssertionError();
        }
        if (isText()) {
            int cch = cchLeft();
            CharNode cn = getCharNodes();
            while (true) {
                int i = cch - cn._cch;
                cch = i;
                if (i >= 0) {
                    cn = cn._next;
                } else {
                    return cn;
                }
            }
        } else {
            return this._xobj.getDom();
        }
    }

    public void release() {
        if (this._tempFrame >= 0) {
            if (this._nextTemp != null) {
                this._nextTemp._prevTemp = this._prevTemp;
            }
            if (this._prevTemp == null) {
                this._locale._tempFrames[this._tempFrame] = this._nextTemp;
            } else {
                this._prevTemp._nextTemp = this._nextTemp;
            }
            this._nextTemp = null;
            this._prevTemp = null;
            this._tempFrame = -1;
        }
        if (this._state != 0 && this._state != 3) {
            while (this._stackTop != -1) {
                popButStay();
            }
            clearSelection();
            this._id = null;
            moveToCur(null);
            if (!isNormal()) {
                throw new AssertionError();
            }
            if (this._xobj != null) {
                throw new AssertionError();
            }
            if (this._pos != -2) {
                throw new AssertionError();
            }
            if (this._ref != null) {
                this._ref.clear();
                this._ref._cur = null;
            }
            this._ref = null;
            if (this._state != 1) {
                throw new AssertionError();
            }
            this._locale._registered = listRemove(this._locale._registered);
            if (this._locale._curPoolCount < 16) {
                this._locale._curPool = listInsert(this._locale._curPool);
                this._state = 0;
                this._locale._curPoolCount++;
                return;
            }
            this._locale = null;
            this._state = 3;
        }
    }

    boolean isOnList(Cur head) {
        while (head != null) {
            if (head != this) {
                head = head._next;
            } else {
                return true;
            }
        }
        return false;
    }

    /* JADX INFO: Access modifiers changed from: package-private */
    public Cur listInsert(Cur head) {
        if (this._next != null || this._prev != null) {
            throw new AssertionError();
        }
        if (head == null) {
            this._prev = this;
            return this;
        }
        this._prev = head._prev;
        head._prev._next = this;
        head._prev = this;
        return head;
    }

    /* JADX INFO: Access modifiers changed from: package-private */
    public Cur listRemove(Cur head) {
        if (this._prev == null || !isOnList(head)) {
            throw new AssertionError();
        }
        if (this._prev == this) {
            head = null;
        } else {
            if (head == this) {
                head = this._next;
            } else {
                this._prev._next = this._next;
            }
            if (this._next == null) {
                if (head != null) {
                    head._prev = this._prev;
                }
            } else {
                this._next._prev = this._prev;
                this._next = null;
            }
        }
        this._prev = null;
        if (this._next != null) {
            throw new AssertionError();
        }
        return head;
    }

    /* JADX INFO: Access modifiers changed from: package-private */
    public boolean isNormal() {
        if (this._state == 0 || this._state == 3) {
            return false;
        }
        if (this._xobj == null) {
            return this._pos == -2;
        }
        if (!this._xobj.isNormal(this._pos)) {
            return false;
        }
        if (this._state == 2) {
            return isOnList(this._xobj._embedded);
        }
        if (this._state != 1) {
            throw new AssertionError();
        }
        return isOnList(this._locale._registered);
    }

    /* loaded from: classes11.dex */
    public static final class CurLoadContext extends Locale.LoadContext {
        static final /* synthetic */ boolean $assertionsDisabled = false;
        private final Map<String, String> _additionalNamespaces;
        private boolean _after;
        private final CharUtil _charUtil;
        private final boolean _discardDocElem;
        private String _doctypeName;
        private String _doctypePublicId;
        private String _doctypeSystemId;
        private Xobj _frontier;
        private int _lastPos;
        private Xobj _lastXobj;
        private final Locale _locale;
        private final QName _replaceDocElem;
        private final boolean _stripComments;
        private boolean _stripLeft = true;
        private final boolean _stripProcinsts;
        private final boolean _stripWhitespace;
        private final Map<String, String> _substituteNamespaces;

        public CurLoadContext(Locale l, XmlOptions options) {
            CharUtil threadLocalCharUtil;
            XmlOptions options2 = XmlOptions.maskNull(options);
            this._locale = l;
            if (options2.isLoadUseLocaleCharUtil()) {
                threadLocalCharUtil = this._locale.getCharUtil();
            } else {
                threadLocalCharUtil = CharUtil.getThreadLocalCharUtil();
            }
            this._charUtil = threadLocalCharUtil;
            this._frontier = Cur.createDomDocumentRootXobj(this._locale);
            this._after = false;
            this._lastXobj = this._frontier;
            this._lastPos = 0;
            this._replaceDocElem = options2.getLoadReplaceDocumentElement();
            this._discardDocElem = options2.hasOption(XmlOptions.XmlOptionsKeys.LOAD_REPLACE_DOCUMENT_ELEMENT);
            this._stripWhitespace = options2.isSetLoadStripWhitespace();
            this._stripComments = options2.isLoadStripComments();
            this._stripProcinsts = options2.isLoadStripProcinsts();
            this._substituteNamespaces = options2.getLoadSubstituteNamespaces();
            this._additionalNamespaces = options2.getLoadAdditionalNamespaces();
            this._locale._versionAll++;
            this._locale._versionSansText++;
        }

        private void start(Xobj xo) {
            if (this._frontier == null) {
                throw new AssertionError();
            }
            if (this._after && this._frontier._parent == null) {
                throw new AssertionError();
            }
            flushText();
            if (this._after) {
                this._frontier = this._frontier._parent;
                this._after = false;
            }
            this._frontier.appendXobj(xo);
            this._frontier = xo;
            this._lastXobj = xo;
            this._lastPos = 0;
        }

        private void end() {
            if (this._frontier == null) {
                throw new AssertionError();
            }
            if (this._after && this._frontier._parent == null) {
                throw new AssertionError();
            }
            flushText();
            if (this._after) {
                this._frontier = this._frontier._parent;
            } else {
                this._after = true;
            }
            this._lastXobj = this._frontier;
            this._lastPos = -1;
        }

        private void text(Object src, int off, int cch) {
            if (cch <= 0) {
                return;
            }
            this._lastXobj = this._frontier;
            this._lastPos = this._frontier._cchValue + 1;
            if (this._after) {
                this._lastPos += this._frontier._cchAfter + 1;
                this._frontier._srcAfter = this._charUtil.saveChars(src, off, cch, this._frontier._srcAfter, this._frontier._offAfter, this._frontier._cchAfter);
                this._frontier._offAfter = this._charUtil._offSrc;
                this._frontier._cchAfter = this._charUtil._cchSrc;
                return;
            }
            this._frontier._srcValue = this._charUtil.saveChars(src, off, cch, this._frontier._srcValue, this._frontier._offValue, this._frontier._cchValue);
            this._frontier._offValue = this._charUtil._offSrc;
            this._frontier._cchValue = this._charUtil._cchSrc;
        }

        private void flushText() {
            if (this._stripWhitespace) {
                if (this._after) {
                    this._frontier._srcAfter = this._charUtil.stripRight(this._frontier._srcAfter, this._frontier._offAfter, this._frontier._cchAfter);
                    this._frontier._offAfter = this._charUtil._offSrc;
                    this._frontier._cchAfter = this._charUtil._cchSrc;
                    return;
                }
                this._frontier._srcValue = this._charUtil.stripRight(this._frontier._srcValue, this._frontier._offValue, this._frontier._cchValue);
                this._frontier._offValue = this._charUtil._offSrc;
                this._frontier._cchValue = this._charUtil._cchSrc;
            }
        }

        private Xobj parent() {
            return this._after ? this._frontier._parent : this._frontier;
        }

        private QName checkName(QName name, boolean local) {
            String substituteUri;
            if (this._substituteNamespaces != null) {
                if ((!local || name.getNamespaceURI().length() > 0) && (substituteUri = this._substituteNamespaces.get(name.getNamespaceURI())) != null) {
                    return this._locale.makeQName(substituteUri, name.getLocalPart(), name.getPrefix());
                }
                return name;
            }
            return name;
        }

        @Override // org.apache.xmlbeans.impl.store.Locale.LoadContext
        protected void startDTD(String name, String publicId, String systemId) {
            this._doctypeName = name;
            this._doctypePublicId = publicId;
            this._doctypeSystemId = systemId;
        }

        @Override // org.apache.xmlbeans.impl.store.Locale.LoadContext
        protected void endDTD() {
        }

        @Override // org.apache.xmlbeans.impl.store.Locale.LoadContext
        protected void startElement(QName name) {
            start(Cur.createElementXobj(this._locale, checkName(name, false), parent()._name));
            this._stripLeft = true;
        }

        @Override // org.apache.xmlbeans.impl.store.Locale.LoadContext
        protected void endElement() {
            if (!parent().isElem()) {
                throw new AssertionError();
            }
            end();
            this._stripLeft = true;
        }

        @Override // org.apache.xmlbeans.impl.store.Locale.LoadContext
        protected void xmlns(String prefix, String uri) {
            String substituteUri;
            if (!parent().isContainer()) {
                throw new AssertionError();
            }
            if (this._substituteNamespaces != null && (substituteUri = this._substituteNamespaces.get(uri)) != null) {
                uri = substituteUri;
            }
            Xobj x = new AttrXobj(this._locale, this._locale.createXmlns(prefix));
            start(x);
            text(uri, 0, uri.length());
            end();
            this._lastXobj = x;
            this._lastPos = 0;
        }

        @Override // org.apache.xmlbeans.impl.store.Locale.LoadContext
        public void attr(QName name, String value) {
            Xobj x;
            if (!parent().isContainer()) {
                throw new AssertionError();
            }
            QName parentName = (this._after ? this._lastXobj._parent : this._lastXobj).getQName();
            boolean isId = isAttrOfTypeId(name, parentName);
            if (isId) {
                x = new AttrIdXobj(this._locale, checkName(name, true));
            } else {
                x = new AttrXobj(this._locale, checkName(name, true));
            }
            start(x);
            text(value, 0, value.length());
            end();
            if (isId) {
                Cur c1 = x.tempCur();
                c1.toRoot();
                Xobj doc = c1._xobj;
                c1.release();
                if (doc instanceof DocumentXobj) {
                    ((DocumentXobj) doc).addIdElement(value, x._parent.getDom());
                }
            }
            this._lastXobj = x;
            this._lastPos = 0;
        }

        @Override // org.apache.xmlbeans.impl.store.Locale.LoadContext
        protected void attr(String local, String uri, String prefix, String value) {
            attr(this._locale.makeQName(uri, local, prefix), value);
        }

        @Override // org.apache.xmlbeans.impl.store.Locale.LoadContext
        protected void procInst(String target, String value) {
            if (!this._stripProcinsts) {
                Xobj x = new ProcInstXobj(this._locale, target);
                start(x);
                text(value, 0, value.length());
                end();
                this._lastXobj = x;
                this._lastPos = 0;
            }
            this._stripLeft = true;
        }

        @Override // org.apache.xmlbeans.impl.store.Locale.LoadContext
        protected void comment(String comment) {
            if (!this._stripComments) {
                comment(comment, 0, comment.length());
            }
            this._stripLeft = true;
        }

        @Override // org.apache.xmlbeans.impl.store.Locale.LoadContext
        protected void comment(char[] chars, int off, int cch) {
            if (!this._stripComments) {
                comment(this._charUtil.saveChars(chars, off, cch), this._charUtil._offSrc, this._charUtil._cchSrc);
            }
            this._stripLeft = true;
        }

        private void comment(Object src, int off, int cch) {
            Xobj x = new CommentXobj(this._locale);
            start(x);
            text(src, off, cch);
            end();
            this._lastXobj = x;
            this._lastPos = 0;
        }

        private void stripText(Object src, int off, int cch) {
            if (this._stripWhitespace && this._stripLeft) {
                src = this._charUtil.stripLeft(src, off, cch);
                this._stripLeft = false;
                off = this._charUtil._offSrc;
                cch = this._charUtil._cchSrc;
            }
            text(src, off, cch);
        }

        @Override // org.apache.xmlbeans.impl.store.Locale.LoadContext
        protected void text(String s) {
            if (s == null) {
                return;
            }
            stripText(s, 0, s.length());
        }

        @Override // org.apache.xmlbeans.impl.store.Locale.LoadContext
        protected void text(char[] src, int off, int cch) {
            stripText(src, off, cch);
        }

        @Override // org.apache.xmlbeans.impl.store.Locale.LoadContext
        protected void bookmark(XmlCursor.XmlBookmark bm) {
            this._lastXobj.setBookmark(this._lastPos, bm.getKey(), bm);
        }

        @Override // org.apache.xmlbeans.impl.store.Locale.LoadContext
        protected void bookmarkLastNonAttr(XmlCursor.XmlBookmark bm) {
            if (this._lastPos > 0 || !this._lastXobj.isAttr()) {
                this._lastXobj.setBookmark(this._lastPos, bm.getKey(), bm);
            } else {
                if (this._lastXobj._parent == null) {
                    throw new AssertionError();
                }
                this._lastXobj._parent.setBookmark(0, bm.getKey(), bm);
            }
        }

        @Override // org.apache.xmlbeans.impl.store.Locale.LoadContext
        protected void bookmarkLastAttr(QName attrName, XmlCursor.XmlBookmark bm) {
            if (this._lastPos == 0 && this._lastXobj.isAttr()) {
                if (this._lastXobj._parent == null) {
                    throw new AssertionError();
                }
                Xobj a = this._lastXobj._parent.getAttr(attrName);
                if (a != null) {
                    a.setBookmark(0, bm.getKey(), bm);
                }
            }
        }

        @Override // org.apache.xmlbeans.impl.store.Locale.LoadContext
        protected void lineNumber(int line, int column, int offset) {
            this._lastXobj.setBookmark(this._lastPos, XmlLineNumber.class, new XmlLineNumber(line, column, offset));
        }

        @Override // org.apache.xmlbeans.impl.store.Locale.LoadContext
        protected void abort() {
            this._stripLeft = true;
            while (!parent().isRoot()) {
                end();
            }
            finish().release();
        }

        /* JADX WARN: Removed duplicated region for block: B:41:0x00a0  */
        @Override // org.apache.xmlbeans.impl.store.Locale.LoadContext
        /*
            Code decompiled incorrectly, please refer to instructions dump.
            To view partially-correct add '--show-bad-code' argument
        */
        public org.apache.xmlbeans.impl.store.Cur finish() {
            /*
                Method dump skipped, instructions count: 331
                To view this dump add '--comments-level debug' option
            */
            throw new UnsupportedOperationException("Method not decompiled: org.apache.xmlbeans.impl.store.Cur.CurLoadContext.finish():org.apache.xmlbeans.impl.store.Cur");
        }

        public void dump() {
            this._frontier.dump();
        }
    }

    static String kindName(int kind) {
        switch (kind) {
            case 0:
                return "TEXT";
            case 1:
                return "ROOT";
            case 2:
                return "ELEM";
            case 3:
                return "ATTR";
            case 4:
                return "COMMENT";
            case 5:
                return "PROCINST";
            default:
                return "<< Unknown Kind (" + kind + ") >>";
        }
    }

    /* JADX INFO: Access modifiers changed from: package-private */
    public void dump() {
        dump(System.out, this._xobj, this);
    }

    /* JADX INFO: Access modifiers changed from: package-private */
    public void dump(PrintStream o) {
        if (this._xobj == null) {
            o.println("Unpositioned xptr");
        } else {
            dump(o, this._xobj, this);
        }
    }

    public static void dump(PrintStream o, Xobj xo, Object ref) {
        if (ref == null) {
            ref = xo;
        }
        while (xo._parent != null) {
            xo = xo._parent;
        }
        dumpXobj(o, xo, 0, ref);
        o.println();
    }

    private static void dumpCur(PrintStream o, String prefix, Cur c, Object ref) {
        o.print(StringUtils.SPACE);
        if (ref == c) {
            o.print("*:");
        }
        o.print(prefix + (c._id == null ? "<cur>" : c._id) + CollectionUtils.DEFAULT_TOSTRING_PREFIX + c._pos + CollectionUtils.DEFAULT_TOSTRING_SUFFIX);
    }

    private static void dumpCurs(PrintStream o, Xobj xo, Object ref) {
        for (Cur c = xo._embedded; c != null; c = c._next) {
            dumpCur(o, "E:", c, ref);
        }
        for (Cur c2 = xo._locale._registered; c2 != null; c2 = c2._next) {
            if (c2._xobj == xo) {
                dumpCur(o, "R:", c2, ref);
            }
        }
    }

    private static void dumpBookmarks(PrintStream o, Xobj xo, Object ref) {
        for (Bookmark b = xo._bookmarks; b != null; b = b._next) {
            o.print(StringUtils.SPACE);
            if (ref == b) {
                o.print("*:");
            }
            if (b._value instanceof XmlLineNumber) {
                XmlLineNumber l = (XmlLineNumber) b._value;
                o.print("<line:" + l.getLine() + ">[" + b._pos + CollectionUtils.DEFAULT_TOSTRING_SUFFIX);
            } else {
                o.print("<mark>[" + b._pos + CollectionUtils.DEFAULT_TOSTRING_SUFFIX);
            }
        }
    }

    private static void dumpCharNodes(PrintStream o, CharNode nodes, Object ref) {
        for (CharNode n = nodes; n != null; n = n._next) {
            o.print(StringUtils.SPACE);
            if (n == ref) {
                o.print("*");
            }
            o.print((n instanceof TextNode ? "TEXT" : "CDATA") + CollectionUtils.DEFAULT_TOSTRING_PREFIX + n._cch + CollectionUtils.DEFAULT_TOSTRING_SUFFIX);
        }
    }

    private static void dumpChars(PrintStream o, Object src, int off, int cch) {
        o.print("\"");
        String s = CharUtil.getString(src, off, cch);
        int i = 0;
        while (true) {
            if (i >= s.length()) {
                break;
            }
            if (i == 36) {
                o.print("...");
                break;
            }
            int codePoint = s.codePointAt(i);
            char[] chars = Character.toChars(codePoint);
            if (chars.length == 1) {
                char ch = chars[0];
                if (ch >= ' ' && ch < 127 && ch != '\"') {
                    o.print(ch);
                } else if (ch == '\n') {
                    o.print("\\n");
                } else if (ch == '\r') {
                    o.print("\\r");
                } else if (ch == '\t') {
                    o.print("\\t");
                } else if (ch == '\"') {
                    o.print("\\\"");
                } else {
                    o.print("<#" + ((int) ch) + ">");
                }
            } else {
                o.print("<#" + codePoint + ">");
            }
            i += Character.charCount(codePoint);
        }
        o.print("\"");
    }

    private static void dumpXobj(PrintStream o, Xobj xo, int level, Object ref) {
        int i;
        if (xo == null) {
            return;
        }
        if (xo == ref) {
            o.print("* ");
        } else {
            o.print("  ");
        }
        for (int i2 = 0; i2 < level; i2++) {
            o.print("  ");
        }
        o.print(kindName(xo.kind()));
        if (xo._name != null) {
            o.print(StringUtils.SPACE);
            if (xo._name.getPrefix().length() > 0) {
                o.print(xo._name.getPrefix() + ":");
            }
            o.print(xo._name.getLocalPart());
            if (xo._name.getNamespaceURI().length() > 0) {
                o.print("@" + xo._name.getNamespaceURI());
            }
        }
        if (xo._srcValue != null || xo._charNodesValue != null) {
            o.print(" Value( ");
            dumpChars(o, xo._srcValue, xo._offValue, xo._cchValue);
            dumpCharNodes(o, xo._charNodesValue, ref);
            o.print(" )");
        }
        if (xo._user != null) {
            o.print(" (USER)");
        }
        if (xo.isVacant()) {
            o.print(" (VACANT)");
        }
        if (xo._srcAfter != null || xo._charNodesAfter != null) {
            o.print(" After( ");
            dumpChars(o, xo._srcAfter, xo._offAfter, xo._cchAfter);
            dumpCharNodes(o, xo._charNodesAfter, ref);
            o.print(" )");
        }
        dumpCurs(o, xo, ref);
        dumpBookmarks(o, xo, ref);
        String className = xo.getClass().getName();
        int i3 = className.lastIndexOf(46);
        if (i3 > 0 && (i = (className = className.substring(i3 + 1)).lastIndexOf(36)) > 0) {
            className = className.substring(i + 1);
        }
        o.print(" (");
        o.print(className);
        o.print(")");
        o.println();
        for (Xobj xo2 = xo._firstChild; xo2 != null; xo2 = xo2._nextSibling) {
            dumpXobj(o, xo2, level + 1, ref);
        }
    }

    void setId(String id) {
        this._id = id;
    }

    public Locale getLocale() {
        return this._locale;
    }
}
