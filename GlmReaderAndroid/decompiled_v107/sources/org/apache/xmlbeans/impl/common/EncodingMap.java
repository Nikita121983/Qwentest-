...
public class EncodingMap {
    ...
    private static void addMapping(String java, String iana, boolean isDefault) {
        if (_iana_to_java.containsKey(iana)) {
            throw new AssertionError();
        }
        if (!java.toUpperCase(Locale.ROOT).equals(java)) {
            throw new AssertionError();
        }
        if (!iana.toUpperCase(Locale.ROOT).equals(iana)) {
            throw new AssertionError();
        }
        _iana_to_java.put(iana, java);
        if (isDefault) {
            if (_java_to_iana.containsKey(java)) {
                throw new AssertionError();
            }
    ...
    private static boolean completeMappings() {
        ...
        for (String k : m.keySet()) {
            if (!_java_to_iana.containsKey(k)) {
