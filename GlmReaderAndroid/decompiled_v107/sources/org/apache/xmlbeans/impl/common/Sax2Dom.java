...
public class Sax2Dom extends DefaultHandler implements ContentHandler, LexicalHandler {
    ...
    public void startElement(String namespace, String localName, String qName, Attributes attrs) {
        ...
        if (this._namespaceDecls != null) {
            ...
            while (i < nDecls) {
                int i2 = i + 1;
                String prefix = this._namespaceDecls.elementAt(i);
                if (prefix == null || prefix.equals("")) {
                    tmp.setAttributeNS("http://www.w3.org/2000/xmlns/", XMLNS_PREFIX, this._namespaceDecls.elementAt(i2));
                } else {
                    tmp.setAttributeNS("http://www.w3.org/2000/xmlns/", XMLNS_STRING + prefix, this._namespaceDecls.elementAt(i2));
                }
                i = i2 + 1;
            }
        ...
        for (int i3 = 0; i3 < nattrs; i3++) {
            if (attrs.getLocalName(i3) == null) {
                tmp.setAttribute(attrs.getQName(i3), attrs.getValue(i3));
            } else {
                tmp.setAttributeNS(attrs.getURI(i3), attrs.getQName(i3), attrs.getValue(i3));
            }
        }
        Node last = this._nodeStk.peek();
