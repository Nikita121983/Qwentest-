...
public class RussianDollStrategy implements XsdGenStrategy {
    ...
    public Element addGlobalElement(Element withElem, TypeSystemHolder typeSystemHolder, Inst2XsdOptions options) {
        if (!withElem.isGlobal()) {
        ...
        while (true) {
            ...
            switch (tt.intValue()) {
                ...
                case 4:
                    ...
                    if (comment == null) {
                        sb = commentBuff.length() == 0 ? null : commentBuff.toString();
                    } else {
                        sb = commentBuff.length() == 0 ? comment : commentBuff.insert(0, comment).toString();
                    }
                    String commnetStr = sb;
                    ...
                    if (!children.isEmpty()) {
                        ...
                        try {
                            ...
                            if (xcForNamespaces != null) {
                                xcForNamespaces.close();
                            }
                        } finally {
                        }
                    }
    ...
    protected void processElementsInComplexType(Type elemType, List<Element> children, String parentNamespace, TypeSystemHolder typeSystemHolder, Inst2XsdOptions options) {
        ...
        for (Element child : children) {
            if (currentElem == null) {
                ...
                currentElem = child;
            } else if (currentElem.getName() != null && currentElem.getName().equals(child.getName())) {
                        ...
                        try {
                            ...
                            try {
                                ...
                                if (idx >= 0 && idx == lexicalValue.lastIndexOf(58) && idx + 1 < lexicalValue.length()) {
                                    ...
                                    xc.getClass();
                                    XmlQNameImpl.validateLexical(lexicalValue, sCTValidationContext, new RussianDollStrategy$$ExternalSyntheticLambda0(xc));
                                    if (this._validationContext.isValid()) {
    ...
    /* JADX INFO: Access modifiers changed from: protected */
    public void combineTypes(Type into, Type with, Inst2XsdOptions options) {
        if (into == with) {
            return;
        }
    ...
    protected void combineAttributesOfTypes(Type into, Type from) {
        ...
        for (int i2 = 0; i2 < into.getAttributes().size(); i2++) {
            Attribute intoAtt2 = into.getAttributes().get(i2);
            for (int j2 = 0; j2 < from.getAttributes().size(); j2++) {
                if (from.getAttributes().get(j2).getName().equals(intoAtt2.getName())) {
                }
            }
    ...
    protected void combineElementsOfTypes(Type into, Type from, Inst2XsdOptions options) {
        boolean needsUnboundedChoice = (into.getTopParticleForComplexOrMixedContent() == 1 && from.getTopParticleForComplexOrMixedContent() == 1) ? false : true;
            ...
            if (fromMatchedIndex < fromStartingIndex) {
                ...
                if (intoMatchedIndex < i) {
                    for (int j3 = fromStartingIndex; j3 < fromMatchedIndex; j3++) {
                        Element fromCandidate2 = from.getElements().get(j3);
                    ...
                    int fromStartingIndex2 = fromMatchedIndex + 1;
                    fromStartingIndex = fromStartingIndex2;
                } else {
        ...
        for (int j4 = fromStartingIndex; j4 < from.getElements().size(); j4++) {
            Element remainingFromElement = from.getElements().get(j4);
        ...
        if (needsUnboundedChoice) {
            ...
            for (int j5 = 0; j5 < from.getElements().size(); j5++) {
                Element fromElem = from.getElements().get(j5);
                int i3 = 0;
                while (true) {
                    if (i3 < into.getElements().size()) {
                        Element intoElem = into.getElements().get(i3);
