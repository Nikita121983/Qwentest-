...
public class SortedArrayStringMap implements IndexedStringMap {
    ...
    private static final TriConsumer<String, Object, StringMap> PUT_ALL = new TriConsumer() { // from class: org.apache.logging.log4j.util.SortedArrayStringMap$$ExternalSyntheticLambda0
    ...
    private void merge(final SortedArrayStringMap other) {
        ...
        for (int i = this.size; i < newSize; i++) {
            ...
            if (index < 0) {
                insertAt(~index, this.keys[i], this.values[i]);
            } else if (overwrite) {
    ...
    @Override // org.apache.logging.log4j.util.StringMap
    public int hashCode() {
        int result = (37 * 31) + this.size;
        return (((result * 31) + hashCode(this.keys, this.size)) * 31) + hashCode(this.values, this.size);
    }
