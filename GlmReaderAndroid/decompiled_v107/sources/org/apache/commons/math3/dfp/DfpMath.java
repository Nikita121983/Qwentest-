...
public class DfpMath {
    ...
    protected static Dfp[] split(Dfp a) {
        ...
        Dfp[] result = {a.add(shift).subtract(shift), a.subtract(result[0])};
        return result;
    }

    protected static Dfp[] splitMult(Dfp[] a, Dfp[] b) {
        Dfp[] result = {a[0].multiply(b[0]), a[0].getZero()};
    ...
    public static Dfp pow(Dfp x, Dfp y) {
        ...
        if (y.lessThan(x.newInstance(100000000)) && y.greaterThan(x.newInstance(-100000000))) {
            ...
            if (v.unequal(zero)) {
                ...
                Dfp c = a2.subtract(b.multiply(x.getField().getLn2()));
    ...
    public static Dfp sin(Dfp a) {
        ...
        if (x.lessThan(pi.divide(4))) {
            ...
            Dfp[] c = {piSplit[0].divide(2).subtract(x), piSplit[1].divide(2)};
            y = cosInternal(c);
        }
        if (neg) {
            y = y.negate();
    ...
    public static Dfp cos(Dfp a) {
        ...
        if (x.lessThan(pi.divide(4))) {
            ...
            Dfp[] piSplit = a.getField().getPiSplit();
            Dfp[] c2 = {piSplit[0].divide(2).subtract(x), piSplit[1].divide(2)};
            y = sinInternal(c2);
        }
        if (neg) {
            y = y.negate();
    ...
    public static Dfp atan(Dfp a) {
        ...
        if (sub) {
            y = y.add(piSplit[c].divide(8)).add(piSplit[c2].divide(8));
        }
        if (recp) {
            y = piSplit[c].divide(2).subtract(y).add(piSplit[c2].divide(2));
        }
        if (neg) {
            y = y.negate();
        }
