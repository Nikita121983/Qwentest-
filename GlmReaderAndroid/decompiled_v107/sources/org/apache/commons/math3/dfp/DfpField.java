...
public class DfpField implements Field<Dfp> {
    ...
    private static Dfp computePi(Dfp one, Dfp two, Dfp three) {
        ...
        while (i < 20) {
            ...
            Dfp p = one.add(yk);
            Dfp p2 = p.multiply(p);
            Dfp multiply = ak.multiply(p2.multiply(p2));
            Dfp multiply2 = two2kp3.multiply(yk);
            Dfp sqrt22 = sqrt2;
            Dfp sqrt23 = one.add(yk);
            Dfp four2 = four;
            Dfp four3 = yk.multiply(yk);
            ak = multiply.subtract(multiply2.multiply(sqrt23.add(four3)));
            if (yk.equals(ykM1)) {
                break;
            }
            i++;
