...
public class AdamsNordsieckTransformer {
    ...
    private AdamsNordsieckTransformer(int n) {
        ...
        BigFraction[] bigC1 = pSolver.solve(new ArrayFieldVector<>((FieldElement[]) u, false)).toArray();
        ...
        FieldMatrix<BigFraction> bigMSupdate = pSolver.solve(new Array2DRowFieldMatrix<>((FieldElement[][]) shiftedP, false));
    ...
    private FieldMatrix<BigFraction> buildP(int rows) {
        BigFraction[][] pData = (BigFraction[][]) Array.newInstance((Class<?>) BigFraction.class, rows, rows);
    ...
    public Array2DRowRealMatrix initializeHighOrderDerivatives(double h, double[] t, double[][] y, double[][] yDot) {
        ...
        char c = 2;
        char c2 = 0;
        double[][] a = (double[][]) Array.newInstance((Class<?>) Double.TYPE, this.c1.length + 1, this.c1.length + 1);
        double[][] b = (double[][]) Array.newInstance((Class<?>) Double.TYPE, this.c1.length + 1, dArr[0].length);
        double[] y0 = dArr[0];
        double[] yDot0 = yDot[0];
        int i = 1;
        while (i < dArr.length) {
