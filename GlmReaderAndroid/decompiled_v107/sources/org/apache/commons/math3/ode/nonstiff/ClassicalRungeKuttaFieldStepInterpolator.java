...
class ClassicalRungeKuttaFieldStepInterpolator<T extends RealFieldElement<T>> extends RungeKuttaFieldStepInterpolator<T> {
    ...
    public ClassicalRungeKuttaFieldStepInterpolator(Field<T> field, boolean forward, T[][] yDotK, FieldODEStateAndDerivative<T> globalPreviousState, FieldODEStateAndDerivative<T> globalCurrentState, FieldODEStateAndDerivative<T> softPreviousState, FieldODEStateAndDerivative<T> softCurrentState, FieldEquationsMapper<T> mapper) {
    ...
    protected FieldODEStateAndDerivative<T> computeInterpolatedStateAndDerivatives(FieldEquationsMapper<T> mapper, T time, T theta, T thetaH, T oneMinusThetaH) {
        ...
        if (getGlobalPreviousState() != null && theta.getReal() <= 0.5d) {
            ...
            currentStateLinearCombination = previousStateLinearCombination(realFieldElement9, realFieldElement10, realFieldElement10, (RealFieldElement) realFieldElement8.multiply((RealFieldElement) realFieldElement7.subtract((RealFieldElement) theta.multiply(3))));
            ...
            currentStateLinearCombination = currentStateLinearCombination(realFieldElement13, realFieldElement14, realFieldElement14, (RealFieldElement) realFieldElement12.multiply((RealFieldElement) ((RealFieldElement) theta.multiply(((RealFieldElement) realFieldElement11.negate()).subtract(1.0d))).subtract(1.0d)));
            derivativeLinearCombination = derivativeLinearCombination(realFieldElement4, realFieldElement5, realFieldElement5, realFieldElement6);
        }
