...
class MidpointFieldStepInterpolator<T extends RealFieldElement<T>> extends RungeKuttaFieldStepInterpolator<T> {
    ...
    public MidpointFieldStepInterpolator(Field<T> field, boolean forward, T[][] yDotK, FieldODEStateAndDerivative<T> globalPreviousState, FieldODEStateAndDerivative<T> globalCurrentState, FieldODEStateAndDerivative<T> softPreviousState, FieldODEStateAndDerivative<T> softCurrentState, FieldEquationsMapper<T> mapper) {
    ...
    protected FieldODEStateAndDerivative<T> computeInterpolatedStateAndDerivatives(FieldEquationsMapper<T> mapper, T time, T theta, T thetaH, T oneMinusThetaH) {
        ...
        if (getGlobalPreviousState() != null && theta.getReal() <= 0.5d) {
            currentStateLinearCombination = previousStateLinearCombination((RealFieldElement) theta.multiply(oneMinusThetaH), (RealFieldElement) theta.multiply(thetaH));
            derivativeLinearCombination = derivativeLinearCombination(realFieldElement2, realFieldElement);
        } else {
            currentStateLinearCombination = currentStateLinearCombination((RealFieldElement) oneMinusThetaH.multiply(theta), (RealFieldElement) ((RealFieldElement) oneMinusThetaH.multiply(theta.add(1.0d))).negate());
            derivativeLinearCombination = derivativeLinearCombination(realFieldElement2, realFieldElement);
        }
