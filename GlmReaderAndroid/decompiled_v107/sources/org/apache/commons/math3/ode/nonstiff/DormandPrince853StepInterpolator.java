...
class DormandPrince853StepInterpolator extends RungeKuttaStepInterpolator {
    ...
    DormandPrince853StepInterpolator(DormandPrince853StepInterpolator interpolator) {
        ...
        this.v = new double[7];
        for (int k2 = 0; k2 < this.v.length; k2++) {
            this.v[k2] = new double[dimension];
            System.arraycopy(interpolator.v[k2], 0, this.v[k2], 0, dimension);
        }
    ...
    protected void computeInterpolatedStateAndDerivatives(double d, double d2) throws MaxCountExceededException {
        ...
        double d20 = (((d16 - 3.0d) * d) + 1.0d) * d16;
        double d21 = ((((5.0d * d) - 8.0d) * d) + 3.0d) * d17;
        double d22 = (((((15.0d - (6.0d * d)) * d) - 12.0d) * d) + 3.0d) * d17;
        double d23 = d17 * d * (((((18.0d - (7.0d * d)) * d) - 15.0d) * d) + 4.0d);
        if (this.previousState != null && d <= 0.5d) {
            ...
            while (true) {
                ...
                if (i4 < this.interpolatedState.length) {
                    ...
                    char c7 = c4;
                    boolean z2 = z;
                    this.interpolatedState[i4] = this.previousState[i4] + (this.h * d * (this.v[z2 ? 1 : 0][i4] + ((this.v[1][i4] + ((this.v[c7][i4] + ((this.v[c6][i4] + ((this.v[c][i4] + ((this.v[c5][i4] + (this.v[6][i4] * d)) * d15)) * d)) * d15)) * d)) * d15)));
                    this.interpolatedDerivatives[i4] = this.v[z2 ? 1 : 0][i4] + (this.v[1][i4] * d18) + (this.v[c7][i4] * d19) + (this.v[c6][i4] * d20) + (this.v[c][i4] * d21) + (this.v[c5][i4] * d22) + (this.v[6][i4] * d23);
                    i4++;
                    c2 = c5;
                    c3 = c6;
                    z = z2 ? 1 : 0;
            ...
            for (int i5 = 0; i5 < this.interpolatedState.length; i5++) {
                this.interpolatedState[i5] = this.currentState[i5] - ((this.v[0][i5] - ((this.v[1][i5] + ((this.v[2][i5] + ((this.v[3][i5] + ((this.v[c][i5] + ((this.v[5][i5] + (this.v[6][i5] * d)) * d15)) * d)) * d15)) * d)) * d)) * d2);
                this.interpolatedDerivatives[i5] = this.v[0][i5] + (this.v[1][i5] * d18) + (this.v[2][i5] * d19) + (this.v[3][i5] * d20) + (this.v[c][i5] * d21) + (this.v[5][i5] * d22) + (this.v[6][i5] * d23);
            }
        }
    }
    ...
    protected void doFinalize() throws MaxCountExceededException {
        ...
        while (true) {
            c = '\n';
            c2 = '\t';
            if (j >= this.currentState.length) {
                break;
            }
            double s = (this.yDotK[0][j] * K14_01) + (this.yDotK[5][j] * K14_06) + (this.yDotK[6][j] * K14_07) + (this.yDotK[7][j] * K14_08) + (this.yDotK[8][j] * K14_09) + (this.yDotK[9][j] * K14_10) + (this.yDotK[10][j] * K14_11) + (this.yDotK[11][j] * K14_12) + (this.yDotK[12][j] * K14_13);
            yTmp[j] = this.currentState[j] + (this.h * s);
            j++;
        }
        this.integrator.computeDerivatives((this.h * C14) + pT, yTmp, this.yDotKLast[0]);
        for (int j2 = 0; j2 < this.currentState.length; j2++) {
            double s2 = (this.yDotK[0][j2] * K15_01) + (this.yDotK[5][j2] * K15_06) + (this.yDotK[6][j2] * K15_07) + (this.yDotK[7][j2] * K15_08) + (this.yDotK[8][j2] * K15_09) + (this.yDotK[9][j2] * 0.1521609496625161d) + (this.yDotK[10][j2] * K15_11) + (this.yDotK[11][j2] * K15_12) + (this.yDotK[12][j2] * K15_13) + (this.yDotKLast[0][j2] * K15_14);
            yTmp[j2] = this.currentState[j2] + (this.h * s2);
        }
        this.integrator.computeDerivatives((this.h * C15) + pT, yTmp, this.yDotKLast[1]);
        int j3 = 0;
        while (j3 < this.currentState.length) {
            double s3 = (this.yDotK[0][j3] * K16_01) + (this.yDotK[5][j3] * K16_06) + (this.yDotK[6][j3] * K16_07) + (this.yDotK[7][j3] * K16_08) + (this.yDotK[8][j3] * K16_09) + (this.yDotK[c2][j3] * 0.1521609496625161d) + (this.yDotK[c][j3] * K16_11) + (this.yDotK[11][j3] * K16_12) + (this.yDotK[12][j3] * K16_13) + (this.yDotKLast[0][j3] * K16_14) + (this.yDotKLast[1][j3] * K16_15);
            yTmp[j3] = this.currentState[j3] + (this.h * s3);
            j3++;
            c = '\n';
            c2 = '\t';
        }
        this.integrator.computeDerivatives((this.h * C16) + pT, yTmp, this.yDotKLast[2]);
    }
    ...
    public void readExternal(ObjectInput in) throws IOException, ClassNotFoundException {
        ...
        this.yDotKLast[2] = dimension >= 0 ? new double[dimension] : null;
        for (int i = 0; i < dimension; i++) {
