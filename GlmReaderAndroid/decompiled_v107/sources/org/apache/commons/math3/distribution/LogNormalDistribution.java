...
public class LogNormalDistribution extends AbstractRealDistribution {
    ...
    public LogNormalDistribution(RandomGenerator rng, double scale, double shape, double inverseCumAccuracy) throws NotStrictlyPositiveException {
        ...
        this.scale = scale;
        this.shape = shape;
        this.logShapePlusHalfLog2Pi = FastMath.log(shape) + (FastMath.log(6.283185307179586d) * 0.5d);
    ...
    public double cumulativeProbability(double x) {
        ...
        if (FastMath.abs(dev) > this.shape * 40.0d) {
    ...
    public double getNumericalVariance() {
        double s = this.shape;
        double ss = s * s;
        return FastMath.expm1(ss) * FastMath.exp((this.scale * 2.0d) + ss);
    }
