...
public abstract class AbstractSubHyperplane<S extends Space, T extends Space> implements SubHyperplane<S> {
    ...
    public AbstractSubHyperplane(Hyperplane<S> hyperplane, Region<T> remainingRegion) {
    ...
    public AbstractSubHyperplane<S, T> reunite(SubHyperplane<S> other) {
        ...
        return buildNew(this.hyperplane, new RegionFactory().union(this.remainingRegion, o.remainingRegion));
    }
    ...
    private BSPTree<T> recurseTransform(BSPTree<T> node, Hyperplane<S> transformed, Transform<S, T> transform, Map<BSPTree<T>, BSPTree<T>> map) {
        ...
        if (node.getCut() == null) {
            ...
            if (attribute != null) {
                ...
                SubHyperplane<T> tPI = attribute.getPlusInside() != null ? transform.apply(attribute.getPlusInside(), this.hyperplane, transformed) : null;
