...
public class PearsonsCorrelation {
    ...
    public RealMatrix getCorrelationStandardErrors() {
        ...
        double[][] out = (double[][]) Array.newInstance((Class<?>) Double.TYPE, nVars, nVars);
        for (int i = 0; i < nVars; i++) {
            for (int j = 0; j < nVars; j++) {
                double r = this.correlationMatrix.getEntry(i, j);
                out[i][j] = FastMath.sqrt((1.0d - (r * r)) / (this.nObs - 2));
            }
        }
        ...
        for (int i = 0; i < nVars; i++) {
            for (int j = 0; j < nVars; j++) {
                if (i == j) {
                    ...
                    double r = this.correlationMatrix.getEntry(i, j);
                    double t = FastMath.abs(FastMath.sqrt((this.nObs - 2) / (1.0d - (r * r))) * r);
