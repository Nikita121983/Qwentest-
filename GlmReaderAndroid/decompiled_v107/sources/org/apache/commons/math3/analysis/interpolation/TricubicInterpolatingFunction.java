...
public class TricubicInterpolatingFunction implements TrivariateFunction {
    ...
    public TricubicInterpolatingFunction(double[] x, double[] y, double[] z, double[][][] f, double[][][] dFdX, double[][][] dFdY, double[][][] dFdZ, double[][][] d2FdXdY, double[][][] d2FdXdZ, double[][][] d2FdYdZ, double[][][] d3FdXdYdZ) throws NoDataException, DimensionMismatchException, NonMonotonicSequenceException {
        ...
        int lastJ = yLen - 1;
        int lastK = zLen - 1;
        this.splines = (TricubicFunction[][][]) Array.newInstance((Class<?>) TricubicFunction.class, lastI, lastJ, lastK);
        int j = 0;
        while (j < lastI) {
            if (f[j].length != yLen) {
                int i = j;
            ...
            int ip1 = j + 1;
            int i9 = j;
            double xR = this.xval[ip1] - this.xval[i9];
            int j2 = 0;
            while (j2 < lastJ) {
                int lastJ2 = lastJ;
                ...
                while (k < lastK) {
                    ...
                    double[] beta = {f[i9][j2][k2], f[ip1][j2][k2], f[i9][jp1][k2], f[ip1][jp1][k2], f[i9][j2][kp1], f[ip1][j2][kp1], f[i9][jp1][kp1], f[ip1][jp1][kp1], dFdX[i9][j2][k2] * xR, dFdX[ip1][j2][k2] * xR, dFdX[i9][jp1][k2] * xR, dFdX[ip1][jp1][k2] * xR, dFdX[i9][j2][kp1] * xR, dFdX[ip1][j2][kp1] * xR, dFdX[i9][jp1][kp1] * xR, dFdX[ip1][jp1][kp1] * xR, dFdY[i9][j2][k2] * yR, dFdY[ip1][j2][k2] * yR, dFdY[i9][jp1][k2] * yR, dFdY[ip1][jp1][k2] * yR, dFdY[i9][j2][kp1] * yR, dFdY[ip1][j2][kp1] * yR, dFdY[i9][jp1][kp1] * yR, dFdY[ip1][jp1][kp1] * yR, dFdZ[i9][j2][k2] * zR, dFdZ[ip1][j2][k2] * zR, dFdZ[i9][jp1][k2] * zR, dFdZ[ip1][jp1][k2] * zR, dFdZ[i9][j2][kp1] * zR, dFdZ[ip1][j2][kp1] * zR, dFdZ[i9][jp1][kp1] * zR, dFdZ[ip1][jp1][kp1] * zR, d2FdXdY[i9][j2][k2] * xRyR, d2FdXdY[ip1][j2][k2] * xRyR, d2FdXdY[i9][jp1][k2] * xRyR, d2FdXdY[ip1][jp1][k2] * xRyR, d2FdXdY[i9][j2][kp1] * xRyR, d2FdXdY[ip1][j2][kp1] * xRyR, d2FdXdY[i9][jp1][kp1] * xRyR, d2FdXdY[ip1][jp1][kp1] * xRyR, d2FdXdZ[i9][j2][k2] * xRzR, d2FdXdZ[ip1][j2][k2] * xRzR, d2FdXdZ[i9][jp1][k2] * xRzR, d2FdXdZ[ip1][jp1][k2] * xRzR, d2FdXdZ[i9][j2][kp1] * xRzR, d2FdXdZ[ip1][j2][kp1] * xRzR, d2FdXdZ[i9][jp1][kp1] * xRzR, d2FdXdZ[ip1][jp1][kp1] * xRzR, d2FdYdZ[i9][j2][k2] * yRzR, d2FdYdZ[ip1][j2][k2] * yRzR, d2FdYdZ[i9][jp1][k2] * yRzR, d2FdYdZ[ip1][jp1][k2] * yRzR, d2FdYdZ[i9][j2][kp1] * yRzR, d2FdYdZ[ip1][j2][kp1] * yRzR, d2FdYdZ[i9][jp1][kp1] * yRzR, d2FdYdZ[ip1][jp1][kp1] * yRzR, d3FdXdYdZ[i9][j2][k2] * xRyRzR, d3FdXdYdZ[ip1][j2][k2] * xRyRzR, d3FdXdYdZ[i9][jp1][k2] * xRyRzR, d3FdXdYdZ[ip1][jp1][k2] * xRyRzR, d3FdXdYdZ[i9][j2][kp1] * xRyRzR, d3FdXdYdZ[ip1][j2][kp1] * xRyRzR, d3FdXdYdZ[i9][jp1][kp1] * xRyRzR, d3FdXdYdZ[ip1][jp1][kp1] * xRyRzR};
                    int j3 = j2;
                    this.splines[i9][j3][k2] = new TricubicFunction(computeCoefficients(beta));
                    k = k2 + 1;
                    j2 = j3;
                    lastK = lastK;
                }
