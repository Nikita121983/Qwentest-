...
public class PolynomialsUtils {
    ...
    public static double[] shift(double[] coefficients, double shift) {
        ...
        int[][] coeff = (int[][]) Array.newInstance((Class<?>) Integer.TYPE, dp1, dp1);
        for (int i = 0; i < dp1; i++) {
            for (int j = 0; j <= i; j++) {
        ...
        for (int i3 = 0; i3 < d; i3++) {
            for (int j2 = i3; j2 < d; j2++) {
                int i4 = i3 + 1;
                newCoefficients[i4] = newCoefficients[i4] + (coeff[j2 + 1][j2 - i3] * coefficients[j2 + 1] * FastMath.pow(shift, j2 - i3));
            }
        }
    ...
    private static PolynomialFunction buildPolynomial(int degree, List<BigFraction> coefficients, RecurrenceCoefficientsGenerator generator) {
        synchronized (coefficients) {
            int maxDegree = ((int) FastMath.floor(FastMath.sqrt(coefficients.size() * 2))) - 1;
            if (degree > maxDegree) {
