...
public class Complex implements FieldElement<Complex>, Serializable {
    ...
    public List<Complex> nthRoot(int n) throws NotPositiveException {
        ...
        double nthRootOfAbs = FastMath.pow(abs(), 1.0d / i);
        double nthPhi = getArgument() / i;
        double slice = 6.283185307179586d / i;
        double innerPart = nthPhi;
        int k = 0;
        while (k < i) {
            double realPart = FastMath.cos(innerPart) * nthRootOfAbs;
            double imaginaryPart = FastMath.sin(innerPart) * nthRootOfAbs;
