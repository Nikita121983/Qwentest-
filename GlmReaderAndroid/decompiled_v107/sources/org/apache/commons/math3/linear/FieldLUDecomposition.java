    ...
    public FieldLUDecomposition(FieldMatrix<T> matrix) {
        ...
        for (int col = 0; col < m; col++) {
            ...
            for (int row2 = 0; row2 < col; row2++) {
                FieldElement[] fieldElementArr = this.lu[row2];
                FieldElement fieldElement = fieldElementArr[col];
                for (int i = 0; i < row2; i++) {
                    fieldElement = (FieldElement) fieldElement.subtract(fieldElementArr[i].multiply(this.lu[i][col]));
                }
                fieldElementArr[col] = fieldElement;
            }
            int nonZero = col;
            for (int row3 = col; row3 < m; row3++) {
                FieldElement[] fieldElementArr2 = this.lu[row3];
                FieldElement fieldElement2 = fieldElementArr2[col];
                for (int i2 = 0; i2 < col; i2++) {
                    fieldElement2 = (FieldElement) fieldElement2.subtract(fieldElementArr2[i2].multiply(this.lu[i2][col]));
                }
            ...
            if (nonZero != col) {
                ...
                for (int i3 = 0; i3 < m; i3++) {
                    T tmp = this.lu[nonZero][i3];
                    this.lu[nonZero][i3] = this.lu[col][i3];
                    this.lu[col][i3] = tmp;
                }
            ...
            for (int row4 = col + 1; row4 < m; row4++) {
                FieldElement[] fieldElementArr3 = this.lu[row4];
