...
public class DiagonalMatrix extends AbstractRealMatrix implements Serializable {
    ...
    public RealMatrix multiply(RealMatrix m) throws DimensionMismatchException {
        ...
        double[][] product = (double[][]) Array.newInstance((Class<?>) Double.TYPE, nRows, nCols);
    ...
    public double[][] getData() {
        int dim = getRowDimension();
        double[][] out = (double[][]) Array.newInstance((Class<?>) Double.TYPE, dim, dim);
        for (int i = 0; i < dim; i++) {
            out[i][i] = this.data[i];
        }
        return out;
    ...
    public boolean isSingular(double threshold) {
        ...
        while (i < this.data.length) {
            ...
            if (!Precision.equals(this.data[i], 0.0d, threshold2)) {
                i++;
