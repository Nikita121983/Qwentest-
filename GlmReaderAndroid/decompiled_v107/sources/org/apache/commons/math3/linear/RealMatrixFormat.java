...
public class RealMatrixFormat {
    ...
    public RealMatrixFormat() {
        this(VectorFormat.DEFAULT_PREFIX, VectorFormat.DEFAULT_SUFFIX, VectorFormat.DEFAULT_PREFIX, VectorFormat.DEFAULT_SUFFIX, CollectionUtils.COMMA, CollectionUtils.COMMA, CompositeFormat.getDefaultNumberFormat());
    }

    public RealMatrixFormat(NumberFormat format) {
        this(VectorFormat.DEFAULT_PREFIX, VectorFormat.DEFAULT_SUFFIX, VectorFormat.DEFAULT_PREFIX, VectorFormat.DEFAULT_SUFFIX, CollectionUtils.COMMA, CollectionUtils.COMMA, format);
    }
    ...
    public RealMatrix parse(String source, ParsePosition pos) {
        ...
        boolean loop2 = rowComponents.isEmpty();
        if (!loop2) {
            matrix.add(rowComponents);
        }
        CompositeFormat.parseAndIgnoreWhitespace(source, pos);
