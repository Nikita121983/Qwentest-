...
public class Conversion {
    ...
    public static char binaryBeMsb0ToHexDigit(boolean[] src, int srcPos) {
        ...
        return (3 > pos || !src[pos + (-3)]) ? (2 > pos || !src[pos + (-2)]) ? (1 > pos || !src[pos + (-1)]) ? src[pos] ? '1' : '0' : src[pos] ? '3' : '2' : src[pos + (-1)] ? src[pos] ? '7' : '6' : src[pos] ? '5' : '4' : src[pos + (-2)] ? src[pos + (-1)] ? src[pos] ? 'f' : 'e' : src[pos] ? 'd' : 'c' : src[pos + (-1)] ? src[pos] ? 'b' : 'a' : src[pos] ? '9' : '8';
    }
    ...
    public static long shortArrayToLong(short[] src, int srcPos, long dstInit, int dstPos, int nShorts) {
        ...
        for (int i = 0; i < nShorts; i++) {
            int shift = (i * 16) + dstPos;
            long bits = (src[i + srcPos] & 65535) << shift;
