...
public final class HmacUtils {
    ...
    public byte[] hmac(Path valueToDigest) throws IOException {
        BufferedInputStream stream = new BufferedInputStream(Files.newInputStream(valueToDigest, new OpenOption[0]));
        try {
            byte[] hmac = hmac(stream);
            stream.close();
            return hmac;
        } catch (Throwable th) {
            try {
    ...
    public String hmacHex(Path valueToDigest) throws IOException {
        return Hex.encodeHexString(hmac(valueToDigest));
    }
