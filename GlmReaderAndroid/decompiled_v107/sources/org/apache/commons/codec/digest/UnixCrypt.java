...
public class UnixCrypt {
    ...
    private static int[] body(int[] schedule, int eSwap0, int eSwap1) {
        ...
        for (int j = 0; j < 25; j++) {
            ...
            int left3 = 0;
            while (left3 < 32) {
                int[] schedule2 = schedule;
                int eSwap02 = eSwap0;
                int eSwap12 = eSwap1;
                int left4 = dEncrypt(left2, right2, left3, eSwap02, eSwap12, schedule2);
                int i = left3;
                right2 = dEncrypt(right2, left4, i + 2, eSwap02, eSwap12, schedule2);
                left2 = left4;
                eSwap0 = eSwap02;
                eSwap1 = eSwap12;
                left3 = i + 4;
        ...
        int t = right;
        int right3 = (left >>> 1) | (left << 31);
        int left5 = (t >>> 1) | (t << 31);
        int[] results = new int[2];
        permOp(right3, left5, 1, 1431655765, results);
        int right4 = results[0];
        int left6 = results[1];
        permOp(left6, right4, 8, 16711935, results);
        int left7 = results[0];
        int right5 = results[1];
        permOp(right5, left7, 2, 858993459, results);
        int right6 = results[0];
        int left8 = results[1];
        permOp(left8, right6, 16, 65535, results);
        int left9 = results[0];
        int right7 = results[1];
        permOp(right7, left9, 4, 252645135, results);
        int right8 = results[0];
        int left10 = results[1];
        int[] out = {left10, right8};
        return out;
    }
    ...
    public static String crypt(byte[] original, String salt) {
        String salt2 = salt;
        if (salt2 == null) {
            salt2 = B64.getRandomSalt(2);
        } else if (!CRYPT_SALT_PATTERN.matcher(salt2).matches()) {
            throw new IllegalArgumentException("Invalid salt value: " + salt2);
        }
        StringBuilder buffer = new StringBuilder("             ");
        char charZero = salt2.charAt(0);
        char charOne = salt2.charAt(1);
    ...
    private static int[] desSetKey(byte[] key) {
        ...
        int d5 = results[0];
        int c6 = results[1];
        int d6 = ((d5 & 255) << 16) | (65280 & d5) | ((16711680 & d5) >>> 16) | (((-268435456) & c6) >>> 4);
        int c7 = c6 & 268435455;
        int j = 0;
        int i3 = 0;
        while (i3 < 16) {
    ...
    private static int fourBytesToInt(byte[] b, int offset) {
        ...
        int value2 = value | (byteToUnsigned(b[offset2]) << 8);
        int offset4 = offset3 + 1;
        int value3 = value2 | (byteToUnsigned(b[offset3]) << 16);
        int i = offset4 + 1;
        return value3 | (byteToUnsigned(b[offset4]) << 24);
    }
