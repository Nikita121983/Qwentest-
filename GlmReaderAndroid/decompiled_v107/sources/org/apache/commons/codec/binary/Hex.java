...
public class Hex implements BinaryEncoder, BinaryDecoder {
    ...
    public static int decodeHex(char[] data, byte[] out, int outOffset) throws DecoderException {
        int len = data.length;
        if ((len & 1) != 0) {
            throw new DecoderException("Odd number of characters %,d.", Integer.valueOf(len));
        }
        int outLen = len >> 1;
    ...
    private static char[] encodeHex(byte[] data, int dataOffset, int dataLen, char[] toDigits, char[] out, int outOffset) {
        ...
        for (int i = dataOffset; i < dataOffset + dataLen; i++) {
            ...
            j = j2 + 1;
            out[j2] = toDigits[data[i] & IntersectionPtg.sid];
        }
        return out;
    }
    ...
    protected static int toDigit(char ch, int index) throws DecoderException {
        ...
        if (digit == -1) {
            throw new DecoderException("Illegal hexadecimal character 0x%02X at index %,d.", Integer.valueOf(ch & 255), Integer.valueOf(index));
        }
        return digit;
    }
    ...
    public String toString() {
        return super.toString() + "[charsetName=" + this.charset + CollectionUtils.DEFAULT_TOSTRING_SUFFIX;
    }
}
