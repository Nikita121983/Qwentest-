    ...
    public static class AVLNode<E> {
        ...
        private AVLNode(Iterator<? extends E> iterator, int start, int end, int absolutePositionOfParent, AVLNode<E> prev, AVLNode<E> next) {
            ...
            if (start < mid) {
                iterator2 = iterator;
                prev2 = this;
                prev2.left = new AVLNode<>(iterator2, start, mid - 1, mid, prev, this);
            } else {
                prev2 = this;
                iterator2 = iterator;
                prev2.leftIsPrevious = true;
                prev2.left = prev;
            }
            prev2.value = iterator2.next();
            prev2.relativePosition = mid - absolutePositionOfParent;
            if (mid >= end) {
                prev2.rightIsNext = true;
                prev2.right = next;
            } else {
                prev2.right = new AVLNode<>(iterator2, mid + 1, end, mid, prev2, next);
            }
            recalcHeight();
        }
        ...
        int indexOf(Object object, int index) {
            int result;
            if (getLeftSubTree() != null && (result = this.left.indexOf(object, this.left.relativePosition + index)) != -1) {
                return result;
            }
            ...
            if (getRightSubTree() != null) {
                return this.right.indexOf(object, this.right.relativePosition + index);
            }
            return -1;
        }
        ...
        private int setOffset(AVLNode<E> node, int newOffset) {
            if (node == null) {
                return 0;
            }
        ...
        void toArray(Object[] array, int index) {
            ...
            if (getLeftSubTree() != null) {
                this.left.toArray(array, this.left.relativePosition + index);
            }
            if (getRightSubTree() != null) {
                this.right.toArray(array, this.right.relativePosition + index);
            }
        }

        public String toString() {
            return "AVLNode(" + this.relativePosition + CollectionUtils.COMMA + (this.left != null) + CollectionUtils.COMMA + this.value + CollectionUtils.COMMA + (getRightSubTree() != null) + this.rightIsNext + ")";
        }
    }
