...
public abstract class AbstractMapBag<E> implements Bag<E> {
    ...
    /* JADX INFO: Access modifiers changed from: protected */
    public AbstractMapBag(Map<E, MutableInteger> map, Iterable<? extends E> iterable) {
        this(map);
        iterable.forEach(new Consumer() { // from class: org.apache.commons.collections4.bag.AbstractMapBag$$ExternalSyntheticLambda0
            @Override // java.util.function.Consumer
            public final void accept(Object obj) {
                AbstractMapBag.this.add(obj);
            }
        });
    }
    ...
    public boolean containsAll(Collection<?> coll) {
        ...
        return containsAll((Bag<?>) new HashBag((Collection) coll));
    }

    /* JADX INFO: Access modifiers changed from: protected */
    /* JADX WARN: Multi-variable type inference failed */
    public void doReadObject(Map<E, MutableInteger> map, ObjectInputStream in) throws IOException, ClassNotFoundException {
        this.map = map;
        int entrySize = in.readInt();
        for (int i = 0; i < entrySize; i++) {
            Object readObject = in.readObject();
            int count = in.readInt();
            map.put(readObject, new MutableInteger(count));
            this.size += count;
        }
    }
    ...
    public boolean removeAll(Collection<?> coll) {
        ...
        if (coll != null) {
            for (Object current : coll) {
                boolean z = true;
                boolean changed = remove(current, 1);
                if (!result && !changed) {
                    z = false;
                }
                result = z;
    ...
    public boolean retainAll(Collection<?> coll) {
        ...
        return retainAll((Bag<?>) new HashBag((Collection) coll));
    }
