...
public class CTDepthPercentImpl extends XmlComplexContentImpl implements CTDepthPercent {
    ...
    public STDepthPercent xgetVal() {
        STDepthPercent target;
        synchronized (monitor()) {
            check_orphaned();
            target = (STDepthPercent) get_store().find_attribute_user(PROPERTY_QNAME[0]);
            if (target == null) {
                target = (STDepthPercent) get_default_attribute_value(PROPERTY_QNAME[0]);
            }
        }
        return target;
    }
    ...
    public void xsetVal(STDepthPercent val) {
        synchronized (monitor()) {
            check_orphaned();
            STDepthPercent target = (STDepthPercent) get_store().find_attribute_user(PROPERTY_QNAME[0]);
            if (target == null) {
                target = (STDepthPercent) get_store().add_attribute_user(PROPERTY_QNAME[0]);
            }
            target.set(val);
        }
    }
