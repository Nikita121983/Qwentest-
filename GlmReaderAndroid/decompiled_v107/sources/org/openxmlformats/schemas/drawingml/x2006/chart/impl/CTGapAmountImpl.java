...
public class CTGapAmountImpl extends XmlComplexContentImpl implements CTGapAmount {
    ...
    public STGapAmount xgetVal() {
        ...
        synchronized (monitor()) {
            check_orphaned();
            target = (STGapAmount) get_store().find_attribute_user(PROPERTY_QNAME[0]);
            if (target == null) {
                target = (STGapAmount) get_default_attribute_value(PROPERTY_QNAME[0]);
            }
        }
        return target;
    }
    ...
    public void xsetVal(STGapAmount val) {
        synchronized (monitor()) {
            check_orphaned();
            STGapAmount target = (STGapAmount) get_store().find_attribute_user(PROPERTY_QNAME[0]);
            if (target == null) {
                target = (STGapAmount) get_store().add_attribute_user(PROPERTY_QNAME[0]);
            }
            target.set(val);
        }
    }
