...
public class CTOrientationImpl extends XmlComplexContentImpl implements CTOrientation {
    ...
    public STOrientation xgetVal() {
        ...
        synchronized (monitor()) {
            check_orphaned();
            target = (STOrientation) get_store().find_attribute_user(PROPERTY_QNAME[0]);
            if (target == null) {
                target = (STOrientation) get_default_attribute_value(PROPERTY_QNAME[0]);
            }
        }
        return target;
    }
    ...
    public void xsetVal(STOrientation val) {
        synchronized (monitor()) {
            check_orphaned();
            STOrientation target = (STOrientation) get_store().find_attribute_user(PROPERTY_QNAME[0]);
            if (target == null) {
                target = (STOrientation) get_store().add_attribute_user(PROPERTY_QNAME[0]);
            }
            target.set(val);
        }
    }
