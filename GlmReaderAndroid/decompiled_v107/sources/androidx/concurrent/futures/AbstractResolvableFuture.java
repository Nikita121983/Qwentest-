...
public abstract class AbstractResolvableFuture<V> implements ListenableFuture<V> {
    ...
    public final V get(long timeout, TimeUnit unit) throws InterruptedException, TimeoutException, ExecutionException {
        ...
        String message = "Waited " + timeout + StringUtils.SPACE + unit.toString().toLowerCase(Locale.ROOT);
        if (remainingNanos + 1000 < j) {
            ...
            if (overWaitUnits > j) {
                String message3 = message2 + overWaitUnits + StringUtils.SPACE + unitString;
                if (shouldShowExtraNanos) {
                    message3 = message3 + CollectionUtils.COMMA;
                }
                message2 = message3 + StringUtils.SPACE;
            }
            if (shouldShowExtraNanos) {
    ...
    public String toString() {
        ...
        if (isCancelled()) {
            ...
            if (pendingDescription != null && !pendingDescription.isEmpty()) {
                builder.append("PENDING, info=[").append(pendingDescription).append(CollectionUtils.DEFAULT_TOSTRING_SUFFIX);
            } else if (isDone()) {
                addDoneString(builder);
            } else {
        ...
        return builder.append(CollectionUtils.DEFAULT_TOSTRING_SUFFIX).toString();
    }
    ...
    protected String pendingToString() {
        Object localValue = this.value;
        if (localValue instanceof SetFuture) {
            return "setFuture=[" + userObjectToString(((SetFuture) localValue).future) + CollectionUtils.DEFAULT_TOSTRING_SUFFIX;
        }
    ...
    private void addDoneString(StringBuilder builder) {
        try {
            builder.append("SUCCESS, result=[").append(userObjectToString(getUninterruptibly(this))).append(CollectionUtils.DEFAULT_TOSTRING_SUFFIX);
        } catch (CancellationException e) {
            builder.append("CANCELLED");
        } catch (RuntimeException e2) {
            builder.append("UNKNOWN, cause=[").append(e2.getClass()).append(" thrown from get()]");
        } catch (ExecutionException e3) {
            builder.append("FAILURE, cause=[").append(e3.getCause()).append(CollectionUtils.DEFAULT_TOSTRING_SUFFIX);
        }
    }
