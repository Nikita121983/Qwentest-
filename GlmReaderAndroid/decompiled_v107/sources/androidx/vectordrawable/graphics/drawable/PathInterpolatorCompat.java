...
public class PathInterpolatorCompat implements Interpolator {
    ...
    private void initPath(Path path) {
        ...
        if (Math.abs(this.mX[0]) > 1.0E-5d || Math.abs(this.mY[0]) > 1.0E-5d || Math.abs(this.mX[numPoints - 1] - 1.0f) > 1.0E-5d || Math.abs(this.mY[numPoints - 1] - 1.0f) > 1.0E-5d) {
            throw new IllegalArgumentException("The Path must start at (0,0) and end at (1,1) start: " + this.mX[0] + CollectionUtils.COMMA + this.mY[0] + " end:" + this.mX[numPoints - 1] + CollectionUtils.COMMA + this.mY[numPoints - 1]);
        }
        float prevX = 0.0f;
