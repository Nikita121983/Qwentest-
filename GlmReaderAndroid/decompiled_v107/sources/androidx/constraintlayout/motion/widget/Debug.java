...
public class Debug {
    ...
    public static void dumpPoc(Object obj) {
        ...
        for (Field declaredField : declaredFields) {
            try {
                ...
                if (declaredField.getName().startsWith("layout_constraint") && ((!(value instanceof Integer) || !value.toString().equals(StructuredDataId.RESERVED)) && ((!(value instanceof Integer) || !value.toString().equals("0")) && ((!(value instanceof Float) || !value.toString().equals("1.0")) && (!(value instanceof Float) || !value.toString().equals("0.5")))))) {
    ...
    public static String getName(Context context, int[] id) {
        String tmp;
        try {
            String str = id.length + CollectionUtils.DEFAULT_TOSTRING_PREFIX;
            int i = 0;
            while (i < id.length) {
                ...
                try {
                    ...
                    tmp = "? " + id[i] + StringUtils.SPACE;
                }
                str = str2 + tmp;
                i++;
            }
            return str + CollectionUtils.DEFAULT_TOSTRING_SUFFIX;
        } catch (Exception ex) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            for (Field declaredField : declaredFields) {
                try {
                    ...
                    if (name.contains("To") && !value.toString().equals(StructuredDataId.RESERVED)) {
                        System.out.println(loc + "       " + declaredField.getName() + StringUtils.SPACE + value);
                    }
                } catch (IllegalAccessException e) {
                }
