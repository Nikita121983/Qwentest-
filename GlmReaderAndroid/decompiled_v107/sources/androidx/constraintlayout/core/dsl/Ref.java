...
public class Ref {
    ...
    public static Ref parseStringToRef(String str) {
        String[] values = str.replaceAll("[\\[\\]\\']", "").split(CollectionUtils.COMMA);
        if (values.length == 0) {
            return null;
        }
    ...
    public String toString() {
        ...
        if (isArray) {
            ret.append(CollectionUtils.DEFAULT_TOSTRING_PREFIX);
        }
        ret.append("'").append(this.mId).append("'");
        if (!Float.isNaN(this.mPostMargin)) {
            ret.append(CollectionUtils.COMMA).append(!Float.isNaN(this.mWeight) ? this.mWeight : 0.0f).append(CollectionUtils.COMMA);
            ret.append(Float.isNaN(this.mPreMargin) ? 0.0f : this.mPreMargin).append(CollectionUtils.COMMA);
            ret.append(this.mPostMargin);
        } else if (!Float.isNaN(this.mPreMargin)) {
            ret.append(CollectionUtils.COMMA).append(Float.isNaN(this.mWeight) ? 0.0f : this.mWeight).append(CollectionUtils.COMMA);
            ret.append(this.mPreMargin);
        } else if (!Float.isNaN(this.mWeight)) {
            ret.append(CollectionUtils.COMMA).append(this.mWeight);
        }
        if (isArray) {
            ret.append(CollectionUtils.DEFAULT_TOSTRING_SUFFIX);
        }
        ret.append(CollectionUtils.COMMA);
        return ret.toString();
    }
}
