...
public class CLContainer extends CLElement {
    ...
    public String toString() {
        ...
        while (it.hasNext()) {
            ...
            if (list.length() > 0) {
                list.append(VectorFormat.DEFAULT_SEPARATOR);
            }
            list.append(element);
        }
    ...
    public CLContainer mo37clone() {
        CLContainer clone = (CLContainer) super.mo37clone();
        ArrayList<CLElement> clonedArray = new ArrayList<>(this.mElements.size());
        Iterator<CLElement> it = this.mElements.iterator();
        while (it.hasNext()) {
            CLElement element = it.next();
            CLElement elementClone = element.mo37clone();
