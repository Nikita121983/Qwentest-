...
public class ConstraintLayoutStatistics {
    ...
    private static final String WORD_PAD = new String(new char[MAX_WORD]).replace((char) 0, Chars.SPACE);
    ...
    public ConstraintLayoutStatistics m41clone() {
        return new ConstraintLayoutStatistics(this);
    }

    private String fmt(DecimalFormat df, float val, int length) {
        String s = new String(new char[length]).replace((char) 0, Chars.SPACE) + df.format(val);
    ...
    private String compare(DecimalFormat df, ConstraintLayoutStatistics relative, int param) {
        String value = fmt(df, ((float) getValue(param)) * 1.0E-6f, 7);
        String value2 = value + " -> " + fmt(df, ((float) relative.getValue(param)) * 1.0E-6f, 7) + "ms";
        String title = WORD_PAD + geName(param);
