...
public class FuncMap {
    ...
    public Function getFunction(String funcName, int numParam) {
        ...
        throw new RuntimeException("function not found: " + funcName + StringUtils.SPACE + numParam);
    }
    ...
    public void setFunction(String funcName, Function f) {
        ...
        if (i2 == this.name.length) {
            String[] tmp1 = new String[this.numFunc * 2];
            Function[] tmp2 = new Function[tmp1.length];
            for (int i3 = 0; i3 < this.numFunc; i3++) {
                tmp1[i3] = this.name[i3];
                tmp2[i3] = this.func[i3];
            }
            this.name = tmp1;
            this.func = tmp2;
        }
