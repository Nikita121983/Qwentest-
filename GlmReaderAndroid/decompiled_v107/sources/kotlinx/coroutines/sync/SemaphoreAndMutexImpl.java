    ...
    */
    ...
    public final boolean addAcquireToQueue(Waiter waiter) {
        ...
        while (true) {
            ...
            Segment to$iv$iv = SegmentOrClosed.m1972getSegmentimpl(s$iv);
            while (true) {
        ...
        SemaphoreSegment segment = (SemaphoreSegment) SegmentOrClosed.m1972getSegmentimpl(s$iv);
        i2 = SemaphoreKt.SEGMENT_SIZE;
        int i3 = (int) (enqIdx % i2);
        if (!AtomicIntrinsicsKt$$ExternalSyntheticBackportWithForwarding0.m(segment.getAcquirers(), i3, null, waiter)) {
            expected$iv = SemaphoreKt.PERMIT;
            value$iv = SemaphoreKt.TAKEN;
            if (AtomicIntrinsicsKt$$ExternalSyntheticBackportWithForwarding0.m(segment.getAcquirers(), i3, expected$iv, value$iv)) {
    ...
    private final boolean tryResumeNextFromQueue() {
        ...
        while (true) {
            ...
            if (!SegmentOrClosed.m1974isClosedimpl(findSegmentInternal)) {
                Segment m1972getSegmentimpl = SegmentOrClosed.m1972getSegmentimpl(findSegmentInternal);
                while (true) {
                    Segment segment = (Segment) atomicReferenceFieldUpdater.get(this);
                    z = false;
                    z2 = true;
                    if (segment.id >= m1972getSegmentimpl.id) {
                        obj = 1;
                        break;
                    }
                    if (!m1972getSegmentimpl.tryIncPointers$kotlinx_coroutines_core()) {
                        obj = null;
                        break;
                    }
                    if (AbstractResolvableFuture$SafeAtomicHelper$$ExternalSyntheticBackportWithForwarding0.m(atomicReferenceFieldUpdater, this, segment, m1972getSegmentimpl)) {
                        if (segment.decPointers$kotlinx_coroutines_core()) {
                            segment.remove();
                        }
                        obj = 1;
                    } else if (m1972getSegmentimpl.decPointers$kotlinx_coroutines_core()) {
                        m1972getSegmentimpl.remove();
                    }
                }
                if (obj != null) {
                    break;
        ...
        SemaphoreSegment semaphoreSegment2 = (SemaphoreSegment) SegmentOrClosed.m1972getSegmentimpl(findSegmentInternal);
