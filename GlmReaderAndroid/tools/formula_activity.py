...
def analyze():
    ...
    for ws_name in wb_form.sheetnames:
            ...
            for col_num, (fc, vc) in enumerate(zip(form_row, val_row), 1):
                if fc.value and isinstance(fc.value, str) and fc.value.startswith("="):
                    ...
                    if result is None or result == "":
                        ...
                        if (
                            cs["sample_result"] is None
                            and len(sheet_info["sample_results"]) < 5
                        ):
        ...
        print(f"\n{'='*70}")
        print(
            f"{ws_name}: {total:,} ф-л → {active:,} ({pct:.0f}%) дают результат, "
            f"{empty:,} ({100-pct:.0f}%) пустые"
        )
        print(
            f"  Столбцы: живые={len(sheet_info['live_columns'])}, "
    ...
    for ws, info in sorted(
        results.items(), key=lambda x: x[1]["formula_cells"], reverse=True
    ):
        t, a, e = (
            info["formula_cells"],
            info["formula_with_value"],
            info["formula_empty"],
        )
        grand_t += t
        grand_a += a
        ...
        print(
            f"  {ws:45s} {t:>7,} → {a:>6,} ({pct:3.0f}%)  пустых:{e:>6,}  "
    ...
    print(f"\n  {'='*70}")
    print(
        f"  ВСЕГО: {grand_t:,} формул → работают: {grand_a:,} ({gp:.0f}%), "
        f"пустые: {grand_e:,} ({100-gp:.0f}%)"
    )
    print(f"  Мёртвых столбцов: {len(all_dead)}")

    with open(
        os.path.join(OUTPUT_DIR, "formula_activity.json"), "w", encoding="utf-8"
    ) as f:
