...
for ws in wb.worksheets:
    for row in ws.iter_rows(min_row=1, max_row=min(ws.max_row, 100), max_col=min(ws.max_column, 50), values_only=False):
        for cell in row:
            if cell.value and isinstance(cell.value, str):
                if 'СОРТ' in cell.value or 'УНИК' in cell.value or 'ФИЛЬТР' in cell.value:
