        ...
        for c in row:
            if c.value and isinstance(c.value, str) and c.value.startswith('='):
                ...
                for ref in sheet_refs:
                    refs.append({'cell': c.coordinate, 'formula': c.value[:150], 'ref_sheet': ref})
    return refs
...
def analyze_process_file():
    ...
    for ws_name in wb.sheetnames:
        ...
        if headers:
            for cell in headers[0].get('cells', []):
                print(f"  {cell['coord']}: {cell['value']} {'[FORMULA]' if cell['formula'] else ''}")
        ...
        if refs:
            ...
            print(f"  [Внешние ссылки в формулах]:")
            for ref_sheet, cnt in ref_counts.most_common(10):
                print(f"    -> {ref_sheet}: {cnt} формул")

        has_formulas = sum(1 for row in ws.iter_rows(min_row=1, values_only=False)
                          for c in row
                          if c.value and isinstance(c.value, str) and c.value.startswith('='))
                ...
                if c.value and isinstance(c.value, str) and c.value.startswith('='):
                    ...
                    for ref in ext_refs:
                        if 'Юнтолово' in ref or 'edit' in ref:
                            cross_refs.append({'cell': c.coordinate, 'sheet': ws_name, 'ref': ref})
...
with open('C:\\Users\\Nik\\SyncthingServiceAcct\\Qwentest\\excel_analysis.json', 'w', encoding='utf-8') as f:
