...
for i in range(1, wb.Sheets.Count + 1):
    ...
    for row in range(1, max_row):
        for col in range(1, max_col):
            ...
            try:
                f = str(cell.Formula)
                if any(
                    kw in f for kw in ["CONCAT", "ДВССЫЛ", "INDIRECT"]
                ) and "HasSpill" not in dir(cell):
    ...
    for col in range(1, max_col):
        ...
        for row in range(1, min(max_row, 10)):
            ...
            try:
                f = str(cell.Formula)
                if f and f != "" and f.startswith("=") is not False:
                    if cell.HasFormula:
                        # Нормализуем
                        import re
        ...
        if col_formulas:
            ...
            sheet_info["column_types"][col_letter] = {
                "type": (
                    "spill"
                    if any(
                        kw in col_formulas[0][2] for kw in ["СОРТ", "УНИК", "ФИЛЬТР"]
                    )
                    else (
                        "dynamic"
                        if any(kw in col_formulas[0][2] for kw in ["CONCAT", "ДВССЫЛ"])
                        else (
                            "index-match"
                            if "INDEX" in col_formulas[0][2]
                            or "ИНДЕКС" in col_formulas[0][2]
                            else "direct" if "!" in col_formulas[0][2] else "calc"
                        )
                    )
                ),
...
# Сохраняем
with open(
    os.path.join(OUTPUT_DIR, "full_logic_analysis.json"), "w", encoding="utf-8"
) as f:
