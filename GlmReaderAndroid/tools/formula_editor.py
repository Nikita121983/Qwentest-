...
def extract_column_formulas(wb):
    ...
    registry = (
        {}
    )  # {sheet: {col: {template: {count, formula, example_rows, formula_text}}}
    ...
    for ws in wb.worksheets:
        ...
        for col_num in range(1, max_col + 1):
            ...
            for row, formula in formulas:
                ...
                tpl = re.sub(
                    r"\$?" + re.escape(col_letter) + r"\$?(\d+)", "$ROW", formula
                )
...
def print_column_summary(registry):
    ...
    print(f"\n{'='*80}")
    print(
        f"ИТОГО: {total_cols} столбцов, {total_multi_variant} с вариантами, {total_formulas:,} формул"
    )
...
def apply_formula_change(sheet_name, col_letter, template_filter, new_formula):
    ...
    for row in range(1, min(ws.max_row + 1, 502)):
        ...
        if cell.value and isinstance(cell.value, str) and cell.value.startswith("="):
            ...
            tpl = re.sub(
                r"\$?" + re.escape(col_letter) + r"\$?(\d+)", "$ROW", cell.value
            )
            tpl2 = re.sub(r"\$?[A-Z]+\$?\d+", "$REF", tpl)
            ...
            if tpl2 == template_filter or template_filter in cell.value:
                ...
                if row_match:
                    ...
                    new_formula_with_row = new_formula.replace(
                        "$ROW", col_letter + old_row_num
                    )
                    ...
                    new_formula_with_row = re.sub(
                        r"(\$?)([A-Z]+)(\$?)(\d+)",
                        lambda m: (
                            m.group(1) + m.group(2) + m.group(3) + str(row)
                            if m.group(2) != col_letter
